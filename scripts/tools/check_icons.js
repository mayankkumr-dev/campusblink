  ...
  for (const icon of imports) {
    ...
    while ((match = iconTagRegex.exec(content)) !== null) {
      ...
      const hardcoded = colorTokens.filter(t => {
        if (t.includes('slate-')) return false; // Handled by theme.css remapping
        if (t.includes('text-[var')) return false; // Handled by CSS variables
        if (t.includes('text-accent')) return false; // Handled by CSS variables
        if (t.includes('text-text-')) return false; // Handled by CSS variables
        return true;
      });
