...
export const StudentChatPage: React.FC = () => {
  ...
  if (!profile?.id) {
    return <div className="p-6 text-sm text-[#6B6B6B]">Please log in to access chat.</div>;
  }

  return (
    <div className="h-full min-h-full bg-[#FAFAF8]">
      <div className="grid h-[calc(100vh-60px)] grid-cols-1 md:grid-cols-[360px_1fr] md:pt-0">
        <aside className="border-r border-[#E8E8E8] bg-white p-4">
          <h1 className="font-syne text-[28px] font-extrabold tracking-tight text-[#0D0D0D]">Chat</h1>

          <div className="mt-3 rounded-md border border-[#E8E8E8] bg-[#FFFCF5] px-3 py-2">
          ...
          <div className="mt-3 grid grid-cols-2 gap-2">
            <button
              ...
              className={`rounded-md px-3 py-2 text-sm font-bold ${viewMode === 'chats' ? 'bg-[#FFD600] text-[#0D0D0D]' : 'border border-[#E8E8E8] bg-white text-[#6B6B6B]'}`}
            >
              Chats ({chats.length})
            </button>
            <button
              ...
              className={`rounded-md px-3 py-2 text-sm font-bold ${viewMode === 'requests' ? 'bg-[#FFD600] text-[#0D0D0D]' : 'border border-[#E8E8E8] bg-white text-[#6B6B6B]'}`}
            >
          ...
          {peopleQuery.trim().length >= 2 ? (
            <div className="mt-3 rounded-md border border-[#E8E8E8] bg-white p-2">
                ...
                peopleResults.map((person) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className={`rounded-full px-2 py-0.5 text-[10px] font-bold ${inNetwork ? 'bg-[#FEF3C7] text-[#A16207]' : 'bg-[#F2F2F2] text-[#6B6B6B]'}`}>
          ...
          <div className="mt-4 h-[calc(100%-230px)] overflow-y-auto">
            {isLoadingConversations ? (
              <div className="rounded-md border border-[#E8E8E8] bg-[#FFFCF5] p-3 text-sm text-[#6D7684]">
              ...
              currentList.map((conversation) => {
                ...
                return (
                  <div key={conversation.id} className={`mb-2 rounded-[10px] border px-3 py-2.5 ${isActive ? 'border-[#EAB308] bg-[#FFFBEB]' : 'border-[#E7EAF1] bg-white hover:bg-[#F8FAFF]'}`}>
                    <button
                      type="button"
                      onClick={() => {
                    ...
                    >
                      <div className="flex items-center justify-between gap-2">
                        ...
                        {unreadCount ? (
                          <span className="inline-flex min-w-6 items-center justify-center rounded-md bg-[#FFD600] px-2 py-1 text-[10px] font-black text-[#0D0D0D]">{unreadCount}</span>
                        ) : null}
                      </div>
                    ...
                    {viewMode === 'requests' ? (
                      <button
                        ...
                        className="mt-2 rounded-md bg-[#0D0D0D] px-3 py-1.5 text-xs font-bold text-white"
                      >
                        Accept request
                      </button>
                    ) : null}
        ...
        <section className="flex h-full flex-col bg-[#FAFAF8]">
          {activeConversation && peerProfile ? (
            <>
              <div className="flex items-center justify-between border-b border-[#E8E8E8] bg-white px-4 py-3.5 md:px-5">
                <div>
                  <button
                    type="button"
              ...
              <div className="flex-1 space-y-3 overflow-y-auto p-4 md:p-5">
                {isLoadingMessages ? (
                  <div className="flex h-full items-center justify-center rounded-md border border-dashed border-[#DCE2EC] bg-white p-6 text-center text-sm text-[#6D7684]">
                  ...
                  messages.map((message) => {
                    ...
                    return (
                      <div key={message.id} className={`flex ${mine ? 'justify-end' : 'justify-start'}`}>
                        <div className={`max-w-[80%] rounded-2xl px-3 py-2 text-sm ${mine ? 'bg-[#0D0D0D] text-white' : 'border border-[#E7EAF1] bg-white text-[#12161F]'}`}>
                          <p>{message.message}</p>
              ...
              <div className="border-t border-[#E7EAF1] bg-white p-3 md:p-4">
                <div className="flex items-center gap-2">
                  <input
                    value={draft}
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-1 rounded-full bg-[#FFD600] px-4 py-2.5 text-sm font-bold text-[#0D0D0D] hover:brightness-95 disabled:opacity-60"
                  >
