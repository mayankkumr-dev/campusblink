  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="rounded-lg border border-black/[0.08] bg-white p-4">
        <h2 className="font-syne text-xl font-bold text-[#0D0D0D]">Contact Issues</h2>
        <p className="mt-1 text-xs uppercase tracking-wider text-[#6B6B6B]">Users submit problems from the Contact page</p>
      </div>
      ...
      <div className="overflow-x-auto rounded-lg border border-black/[0.08] bg-white">
        {isLoading ? (
          ...
          <div className="p-10 text-center text-sm text-[#6B6B6B]">No contact issues found.</div>
        ) : (
          <table className="w-full border-collapse text-left">
            <thead className="bg-[#F5F4F0] h-[40px] border-b border-[#E8E8E8]">
              <tr className="border-b border-black/[0.08] bg-[#F7F5F0] hover:bg-[#FAFAF8] transition-colors duration-150">
                ...
                <th className="p-4 text-xs font-bold uppercase tracking-wider text-[#6B6B6B] px-4 text-left font-sans font-semibold text-[12px] text-[#9B9B9B] uppercase tracking-[0.6px]">Category</th>
                <th className="p-4 text-xs font-bold uppercase tracking-wider text-[#6B6B6B] px-4 text-left font-sans font-semibold text-[12px] text-[#9B9B9B] uppercase tracking-[0.6px]">Issue</th>
                <th className="p-4 text-xs font-bold uppercase tracking-wider text-[#6B6B6B] px-4 text-left font-sans font-semibold text-[12px] text-[#9B9B9B] uppercase tracking-[0.6px]">Status</th>
                <th className="p-4 text-xs font-bold uppercase tracking-wider text-[#6B6B6B] px-4 text-left font-sans font-semibold text-[12px] text-[#9B9B9B] uppercase tracking-[0.6px]">Created</th>
            ...
            <tbody className="divide-y divide-black/[0.06]">
              {filtered.map((issue) => (
                <tr key={issue.id} className="align-top hover:bg-[#FAFAF8]">
                  <td className="p-4 text-sm">
                    <p className="font-bold text-[#0D0D0D]">{issue.name || issue.user?.name || 'Unknown user'}</p>
                    <p className="text-xs text-[#6B6B6B]">{issue.email || issue.user?.email || 'No email'}</p>
                  </td>
                  <td className="p-4 text-xs font-bold uppercase tracking-wider text-[#0D0D0D]">{issue.category || 'general'}</td>
                  <td className="max-w-lg p-4">
                    <p className="text-sm font-bold text-[#0D0D0D]">{issue.subject || 'No subject'}</p>
                    <p className="mt-1 whitespace-pre-wrap text-xs leading-6 text-[#6B6B6B]">{issue.message}</p>
                  </td>
                  <td className="p-4">
                    <span
                      className={`rounded px-2 py-1 text-[10px] font-bold uppercase tracking-wider ${issue.status === 'resolved' ? 'bg-[#DCFCE7] text-[#166534]' : issue.status === 'in_progress' ? 'bg-[#DBEAFE] text-[#1D4ED8]' : 'bg-[#FEF9C3] text-[#92400E]'}`}
                    >
                      {String(issue.status || 'open').replace('_', ' ')}
                    </span>
                  </td>
                  <td className="p-4 text-xs text-[#6B6B6B]">{new Date(issue.created_at).toLocaleString()}</td>
                  <td className="p-4">
                    <div className="flex flex-col gap-2">
                      {issue.status !== 'in_progress' && issue.status !== 'resolved' && (
                        <button
                          onClick={() => setIssueStatus(issue.id, 'in_progress')}
                          className="inline-flex items-center gap-1 rounded bg-[#DBEAFE] px-2.5 py-1.5 text-[10px] font-bold uppercase tracking-wider text-[#1D4ED8]"
                        >
