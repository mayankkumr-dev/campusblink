  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 className="font-syne text-3xl font-extrabold text-[#0D0D0D]">Invite Codes</h1>
          <p className="font-sans text-sm text-[#6B6B6B]">Manage invite access and monitor conversion.</p>
        </div>
        <button
          onClick={() => navigate('/admin/invites/waitlist')}
          className="rounded-md border border-black/10 bg-white px-4 py-2 text-sm font-bold text-[#0D0D0D] hover:bg-[#F2F0EB]"
        >
          View Waitlist
        </button>
      </div>
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        {[
          ...
          <div key={item.label} className="rounded-lg border border-black/10 bg-white p-5 shadow-sm">
            <p className="text-[11px] font-bold uppercase tracking-[0.16em] text-[#6B6B6B]">{item.label}</p>
            <p className="mt-3 font-syne text-4xl font-extrabold text-[#0D0D0D]">{item.value}</p>
          </div>
        ))}
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <div className="rounded-lg border border-black/10 bg-white p-5 space-y-4 shadow-sm">
          <h2 className="font-syne text-xl font-extrabold text-[#0D0D0D]">Generate Codes For User</h2>
          <div className="space-y-3">
            <label className="block">
              <span className="text-xs font-bold uppercase tracking-[0.16em] text-[#6B6B6B]">Search user</span>
              <input
                value={targetUserTerm}
                onChange={(event) => setTargetUserTerm(event.target.value)}
                placeholder="Search by name or email"
                className="mt-1 w-full rounded-lg border border-black/10 px-3 py-2 text-sm outline-none focus:border-[#FFD600]"
              />
            </label>

            {targetUsers.length > 0 ? (
              <div className="max-h-44 overflow-auto rounded-lg border border-black/10 bg-[#FAFAF8] p-2 space-y-1">
                {targetUsers.map((user) => (
                  ...
                  >
                    <p className="font-bold text-[#0D0D0D]">{user.name || 'Unnamed user'}</p>
            ...
            <div className="grid grid-cols-2 gap-3">
              <label className="block">
                <span className="text-xs font-bold uppercase tracking-[0.16em] text-[#6B6B6B]">Number of codes</span>
                <input
                  value={userCodeCount}
              ...
              <label className="block">
                <span className="text-xs font-bold uppercase tracking-[0.16em] text-[#6B6B6B]">Expiry</span>
                <select
                  value={userExpiry}
            ...
            <button
              ...
              className="w-full rounded-md bg-[#0D0D0D] px-4 py-2.5 text-sm font-bold text-white hover:bg-[#FFD600] hover:text-[#0D0D0D]"
            >
        ...
        <div className="rounded-lg border border-black/10 bg-white p-5 space-y-4 shadow-sm">
          <h2 className="font-syne text-xl font-extrabold text-[#0D0D0D]">Bulk Generate Codes</h2>
          <div className="grid grid-cols-2 gap-3">
            <label className="block">
              <span className="text-xs font-bold uppercase tracking-[0.16em] text-[#6B6B6B]">Number of codes</span>
              <input
                value={bulkCount}
                onChange={(event) => setBulkCount(Math.max(1, Math.min(100, Number(event.target.value || 1))))}
                type="number"
                min={1}
                max={100}
                className="mt-1 w-full rounded-lg border border-black/10 px-3 py-2 text-sm outline-none focus:border-[#FFD600]"
          ...
          <label className="block">
            <span className="text-xs font-bold uppercase tracking-[0.16em] text-[#6B6B6B]">Purpose / Note</span>
            <input
              value={bulkNote}
          ...
          <button
            ...
            className="w-full rounded-md bg-[#FFD600] px-4 py-2.5 text-sm font-bold text-[#0D0D0D] hover:bg-[#0D0D0D] hover:text-[#FFD600]"
          >
            Generate Bulk Codes
          </button>

          {generatedCodes.length > 0 ? (
            <div className="rounded-lg border border-black/10 bg-[#FAFAF8] p-3 space-y-2">
              <div className="flex items-center justify-between gap-2">
                <p className="text-xs font-bold uppercase tracking-[0.16em] text-[#6B6B6B]">Generated</p>
              ...
              <div className="max-h-32 overflow-auto space-y-1">
                {generatedCodes.map((code) => (
                  <div key={code} className="rounded-lg bg-white px-3 py-1.5 text-sm font-bold tracking-[0.12em] text-[#0D0D0D]">{code}</div>
                ))}
              </div>
            </div>
          ) : null}
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <div className="rounded-lg border border-black/10 bg-white p-5 shadow-sm lg:col-span-2">
          <div className="flex items-center justify-between gap-3 mb-4">
            <h2 className="font-syne text-xl font-extrabold text-[#0D0D0D]">All Codes</h2>
            <div className="flex items-center gap-2">
          ...
          <div className="mb-3 flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
            <div className="flex items-center gap-2">
              {['all', 'used', 'available', 'expired'].map((item) => (
                <button
                  ...
                  className={`rounded-md px-3 py-1.5 text-xs font-bold uppercase tracking-[0.14em] ${filter === item ? 'bg-[#0D0D0D] text-white' : 'bg-[#FAFAF8] text-[#6B6B6B] border border-black/10'}`}
                >
                  {item}
                </button>
              ))}
            ...
            <label className="relative block w-full md:w-72">
              ...
              <input
                ...
                className="w-full rounded-md border border-black/10 py-2 pl-9 pr-3 text-sm outline-none focus:border-[#FFD600]"
              />
            </label>
          </div>
          ...
          <div className="overflow-auto rounded-lg border border-black/10">
            <table className="min-w-full text-sm">
              <thead className="bg-[#F5F4F0] h-[40px] border-b border-[#E8E8E8]">
                <tr>
                  <th className="px-3 py-2 text-left px-4 text-left font-sans font-semibold text-[12px] text-[#9B9B9B] uppercase tracking-[0.6px]">Code</th>
                  <th className="px-3 py-2 text-left px-4 text-left font-sans font-semibold text-[12px] text-[#9B9B9B] uppercase tracking-[0.6px]">Created By</th>
                  <th className="px-3 py-2 text-left px-4 text-left font-sans font-semibold text-[12px] text-[#9B9B9B] uppercase tracking-[0.6px]">Used By</th>
                  ...
                  <th className="px-3 py-2 text-left px-4 text-left font-sans font-semibold text-[12px] text-[#9B9B9B] uppercase tracking-[0.6px]">Created</th>
                  <th className="px-3 py-2 text-left px-4 text-left font-sans font-semibold text-[12px] text-[#9B9B9B] uppercase tracking-[0.6px]">Expires</th>
              ...
              <tbody>
                  ...
                  return (
                    <tr key={row.id} className="border-t border-black/10">
                      ...
                      <td className="px-3 py-2">
                        <span className={`rounded-md px-2 py-0.5 text-[11px] font-bold ${status === 'Used' ? 'bg-[#F1F5F9] text-[#475569]' : status === 'Expired' ? 'bg-[#FEF2F2] text-[#B91C1C]' : 'bg-[#DCFCE7] text-[#166534]'}`}>
                          {status}
                        </span>
                      </td>
                        ...
                        <div className="inline-flex items-center gap-2">
                          ...
                          {!row.is_used ? (
                            <button
                              ...
                              className="rounded-md border border-[#B91C1C]/30 bg-[#FEF2F2] px-3 py-1 text-xs font-bold text-[#B91C1C]"
                            >
                              Revoke
                            </button>
                          ) : null}
                ...
                {!isLoading && codes.length === 0 ? (
                  <tr>
                    <td className="px-3 py-5 text-center text-sm text-[#6B6B6B]" colSpan={7}>No invite codes found.</td>
                  </tr>
                ) : null}
              </tbody>
            </table>
        ...
        <div className="space-y-6">
          <div className="rounded-lg border border-black/10 bg-white p-5 shadow-sm">
            <h3 className="font-syne text-xl font-extrabold text-[#0D0D0D]">Invite Analytics</h3>
            <p className="mt-2 text-sm text-[#6B6B6B]">Conversion rate: <span className="font-bold text-[#0D0D0D]">{conversionRate}%</span></p>
            <div className="mt-4 space-y-2">
              {invitesPerDay.length > 0 ? invitesPerDay.map((item) => {
                ...
                return (
                  <div key={item.day}>
                    <div className="flex items-center justify-between text-xs text-[#6B6B6B] mb-1"><span>{item.day}</span><span>{item.count}</span></div>
                    <div className="h-2 rounded-md bg-black/10">
                      <div className="h-full rounded-md bg-[#FFD600]" style={{ width: `${width}%` }} />
                    </div>
                  </div>
                );
              }) : <p className="text-sm text-[#6B6B6B]">No invite activity yet.</p>}
            </div>
          </div>

          <div className="rounded-lg border border-black/10 bg-white p-5 shadow-sm">
            <h3 className="font-syne text-xl font-extrabold text-[#0D0D0D]">Top Inviters</h3>
            <div className="mt-4 space-y-2">
              {topInviters.length > 0 ? topInviters.map((item) => (
                <div key={item.id} className="rounded-lg border border-black/10 bg-[#FAFAF8] px-3 py-2.5 flex items-center justify-between">
                  <div>
                    <p className="text-sm font-bold text-[#0D0D0D]">{item.name}</p>
                    <p className="text-xs text-[#6B6B6B]">{item.email}</p>
                  </div>
                  <p className="font-syne text-xl font-extrabold text-[#A16207]">{item.count}</p>
                </div>
              )) : <p className="text-sm text-[#6B6B6B]">No successful invites yet.</p>}
            </div>
          </div>
        </div>
      </div>
