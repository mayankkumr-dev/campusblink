...
export const PrintReorderPage: React.FC = () => {
      ...
      <div className="min-h-screen bg-[#FAFAF8] flex items-center justify-center p-6">
        <div className="max-w-md w-full rounded-lg border border-[#0D0D0D]/10 bg-white p-8 text-center shadow-soft">
          ...
          <button
            ...
            className="px-6 py-3 rounded-md bg-[#FFD600] text-[#0D0D0D] font-bold"
          >
            Go to Login
          </button>
        </div>
  ...
  if (isLoading) {
    return <div className="min-h-screen flex items-center justify-center bg-[#FAFAF8]">Loading reorder request...</div>;
  }
    ...
    <div className="p-4 md:p-8 bg-[#FAFAF8] min-h-full font-sans pb-32">
      <div className="max-w-[1000px] mx-auto">
        <button
          ...
          className="inline-flex items-center gap-2 text-sm font-bold text-[#0D0D0D] hover:text-[#10A142] mb-4"
        >
        ...
        <div className="mb-4 rounded-lg border border-amber-400/40 bg-amber-50 px-5 py-4 flex items-start gap-3">
          ...
          <div>
            <p className="font-bold text-[#0D0D0D] text-sm">Free Reorder Request #{request.id.slice(0, 6)}</p>
            <p className="text-xs text-[#6B6B6B] mt-0.5">Upload your PDF and confirm specs. This reorder is 100% free because your previous order was marked collected by mistake.</p>
            <p className="text-xs text-[#6B6B6B] mt-1">Shop: {shop.name || 'Print Shop'}</p>
          </div>
        </div>

        <div className="bg-white border border-[#0D0D0D]/10 rounded-lg p-5 mb-5">
          <h2 className="font-syne font-bold text-xl text-[#0D0D0D] mb-4">Upload PDF Again</h2>
          <div className="flex flex-col sm:flex-row sm:items-center gap-3">
            ...
            {file ? (
              <div className="inline-flex items-center gap-2 text-sm text-[#0D0D0D] bg-[#F2F0EB] px-3 py-2 rounded-lg">
                <FileText className="w-4 h-4" /> {file.name}
                <button onClick={() => setFile(null)} className="text-[#6B6B6B] hover:text-red-500">
                  <X className="w-4 h-4" />
                </button>
              </div>
            ) : (
              <span className="text-xs text-[#6B6B6B]">No PDF selected yet</span>
            )}
          </div>
        </div>

        <div className="bg-white rounded-lg border border-[#0D0D0D]/10 shadow-medium p-5 md:p-8">
          <h2 className="font-syne font-bold text-xl text-[#0D0D0D] mb-1">Print settings</h2>
          <p className="text-sm text-[#6B6B6B] mb-6">You can change these before final free reorder.</p>

          <div className="flex items-center justify-between gap-4 pb-6 border-b border-[#0D0D0D]/10 mb-6">
            <div>
              <h3 className="font-bold text-[#0D0D0D]">Copies</h3>
            </div>
            <div className="flex items-center justify-between w-[120px] bg-white border border-[#10A142]/30 rounded-lg px-2 py-1 shadow-soft text-[#10A142]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pb-8 border-b border-[#0D0D0D]/10 mb-8">
            <div>
              <h3 className="font-bold text-[#0D0D0D] mb-3">Color Mode</h3>
              <div className="flex gap-3">
                <button onClick={() => setColorMode('bw')} className={`px-4 py-2 rounded-lg border font-bold text-sm ${colorMode === 'bw' ? 'border-[#10A142] bg-[#10A142]/5' : 'border-[#0D0D0D]/10'}`}>
                  B&W (₹{bwPrice}/page)
                </button>
                <button onClick={() => setColorMode('color')} className={`px-4 py-2 rounded-lg border font-bold text-sm ${colorMode === 'color' ? 'border-[#10A142] bg-[#10A142]/5' : 'border-[#0D0D0D]/10'}`}>
            ...
            <div>
              <h3 className="font-bold text-[#0D0D0D] mb-3">Orientation</h3>
              <div className="flex gap-3">
                <button onClick={() => setOrientation('portrait')} className={`px-4 py-2 rounded-lg border font-bold text-sm ${orientation === 'portrait' ? 'border-[#10A142] bg-[#10A142]/5' : 'border-[#0D0D0D]/10'}`}>
                  Portrait
                </button>
                <button onClick={() => setOrientation('landscape')} className={`px-4 py-2 rounded-lg border font-bold text-sm ${orientation === 'landscape' ? 'border-[#10A142] bg-[#10A142]/5' : 'border-[#0D0D0D]/10'}`}>
                  Landscape
                </button>
              </div>
            </div>
          ...
          <div className="bg-[#F2F0EB] rounded-lg border border-[#0D0D0D]/10 p-4 mb-4">
            <p className="text-sm text-[#0D0D0D] font-bold">Normal estimate: ₹{calculateDisplayTotal}</p>
            <p className="text-sm text-[#10A142] font-bold mt-1">You pay now: ₹0 (Free reorder)</p>
            <p className="text-xs text-[#6B6B6B] mt-1">You can leave this page and come back later from the Reorder banner until you submit.</p>
          </div>
