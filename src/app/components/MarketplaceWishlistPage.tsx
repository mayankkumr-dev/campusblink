...
export function MarketplaceWishlistPage() {
  ...
  return (
    <div className="min-h-screen bg-[linear-gradient(180deg,#FBFBF7_0%,#F3EEE2_100%)] px-4 py-5 sm:px-6 lg:px-8">
      <div className="w-full space-y-6">
        <section className="rounded-[34px] border border-black/8 bg-[#0D0D0D] px-6 py-8 text-white shadow-[0_28px_110px_rgba(0,0,0,0.18)] sm:px-8">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div>
              <div className="inline-flex items-center gap-2 rounded-md bg-white/10 px-4 py-2 text-xs font-semibold uppercase tracking-[0.24em] text-[#FFD600]">
                Your shortlist
              </div>
            ...
            <div className="rounded-[28px] bg-white/8 px-6 py-5 text-center ">
              <Heart className="mx-auto h-6 w-6 text-[#FFD600]" />
        ...
        <MarketplaceSectionCard eyebrow="Filter" title="Keep only the items you still want">
          <div className="grid gap-4 lg:grid-cols-[1fr_240px]">
            <input
              ...
              className="w-full rounded-md border border-black/10 bg-[#FFFCF5] px-5 py-3 text-sm outline-none"
            />
            <select
              value={category}
              onChange={(event) => setCategory(event.target.value)}
              className="rounded-md border border-black/10 bg-[#FFFCF5] px-5 py-3 text-sm outline-none"
            >
          ...
          {isLoading ? (
            <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
              {new Array(6).fill(null).map((_, index) => (
                <div key={index} className="overflow-hidden rounded-[30px] border border-black/8 bg-white">
                  <div className="aspect-square animate-pulse bg-[#F0ECE2]" />
                  <div className="space-y-3 p-5">
                    <div className="h-5 w-28 animate-pulse rounded-md bg-[#F0ECE2]" />
                    <div className="h-4 w-full animate-pulse rounded-md bg-[#F0ECE2]" />
                  </div>
                </div>
              ))}
            </div>
