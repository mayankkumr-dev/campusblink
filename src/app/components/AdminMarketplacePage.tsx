...
export const AdminMarketplacePage: React.FC<AdminMarketplacePageProps> = ({ initialFilterStatus = 'All', title }) => {
  ...
  const StatusBadge = ({ status, reports }: { status: string, reports: number }) => {
    ...
    const badges: any = {
      ...
      'sold': 'bg-[#F7F5F0] text-[#0D0D0D]',
      'disabled': 'bg-[#FEE2E2] text-[#DC2626]'
    };
    return (
       <span className={`inline-flex items-center px-2 py-0.5 rounded text-[10px] uppercase font-bold tracking-wider ${badges[normalizedStatus] || 'bg-[#F7F5F0] text-[#0D0D0D]'}`}>
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      {title && (
        <div className="bg-white border border-black/[0.08] rounded-lg p-4">
          <h2 className="font-syne font-bold text-xl text-[#0D0D0D]">{title}</h2>
        </div>
      )}
        ...
        <div className="flex flex-wrap items-center gap-2">
          {['All', 'Active', 'Sold', 'Reported', 'Disabled'].map(pill => (
            <button 
              ...
              className={`px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-wider font-sans transition-colors ${filterStatus === pill ? 'bg-[#FFD600] text-[#0D0D0D]' : pill === 'Reported' && filterStatus === 'Reported' ? 'bg-[#DC2626] text-[#0D0D0D]' : 'bg-[#F7F5F0] text-[#6B6B6B] hover:text-[#0D0D0D] border border-black/[0.08] hover:border-black/10'}`}
            >
              {pill}
            </button>
          ))}
          <select
            ...
            className="ml-2 bg-[#F7F5F0] border border-black/10 rounded-lg py-1.5 px-2 text-[11px] text-[#0D0D0D] font-bold uppercase tracking-wider"
          >
        ...
        <table className="w-full text-left border-collapse">
          <thead className="bg-[#F5F4F0] h-[40px] border-b border-[#E8E8E8]">
            <tr className="border-b border-black/[0.08] bg-[#F7F5F0] hover:bg-[#FAFAF8] transition-colors duration-150">
              <th className="p-4 text-xs font-bold text-[#6B6B6B] uppercase tracking-wider font-sans px-4 text-left font-sans font-semibold text-[12px] text-[#9B9B9B] uppercase tracking-[0.6px]">Listing Title</th>
              <th className="p-4 text-xs font-bold text-[#6B6B6B] uppercase tracking-wider font-sans px-4 text-left font-sans font-semibold text-[12px] text-[#9B9B9B] uppercase tracking-[0.6px]">Seller Details</th>
              <th className="p-4 text-xs font-bold text-[#6B6B6B] uppercase tracking-wider font-sans px-4 text-left font-sans font-semibold text-[12px] text-[#9B9B9B] uppercase tracking-[0.6px]">Price</th>
              <th className="p-4 text-xs font-bold text-[#6B6B6B] uppercase tracking-wider font-sans px-4 text-left font-sans font-semibold text-[12px] text-[#9B9B9B] uppercase tracking-[0.6px]">Status / Health</th>
              ...
              <tr key={listing.id} className="hover:bg-black/[0.03] transition-colors group">
                <td className="p-4 min-w-[250px] max-w-sm">
                  <div className="flex items-center gap-3">
                    <div className="w-12 h-12 rounded-lg bg-[#222222] flex items-center justify-center font-syne font-bold text-2xl text-[#0D0D0D] border border-black/[0.08] shrink-0 shadow-inner overflow-hidden">
                    ...
                    <div className="min-w-0">
                      <div className="font-sans font-bold text-sm text-[#0D0D0D] mb-0.5 truncate pr-4">{listing.title}</div>
                      <div className="font-sans text-[10px] text-[#6B6B6B] uppercase tracking-wider">ID: {listing.id} • {new Date(listing.created_at).toLocaleDateString()}</div>
                    </div>
                  </div>
                </td>
                <td className="p-4">
                   <div className="font-sans text-sm text-[#0D0D0D] font-medium mb-0.5">{listing.seller?.name || 'Unknown User'}</div>
                   <div className="font-sans text-xs text-[#6B6B6B]">{listing.seller?.email || 'No email'}</div>
                </td>
                <td className="p-4">
                ...
                <td className="p-4 text-center relative">
                   <div className="flex items-center gap-2 justify-center">
                     <button title="View Live Post" className="p-1.5 rounded-lg text-[#6B6B6B] hover:text-[#0057FF] hover:bg-[#0057FF]/10 transition-colors">
                       <ExternalLink className="w-4 h-4" />
                     </button>
                     <button 
                        onClick={() => setActiveDropdown(activeDropdown === listing.id ? null : listing.id)}
                        className="p-1.5 rounded-lg bg-[#F7F5F0] text-[#0D0D0D] hover:bg-[#F7F5F0] transition-colors text-xs font-bold font-sans flex items-center gap-1"
                     >
                        Mod <ShieldAlert className="w-3.5 h-3.5 text-[#FFD600]" />
                     </button>
                   </div>

                  {/* Moderation Dropdown */}
                  {activeDropdown === listing.id && (
                    <div className="absolute right-4 top-12 w-48 bg-[#F7F5F0] border border-black/10 rounded-lg shadow-md z-20 py-1 font-sans text-sm overflow-hidden animate-in zoom-in-95 duration-100 text-left">
                      
                      <div className="px-3 py-2 border-b border-black/[0.08]">
                         <span className="text-[10px] font-bold text-[#6B6B6B] uppercase tracking-wider">Danger Zone</span>
                      </div>
                      
                      {!listing.is_sold && !listing.is_admin_disabled && (
                        <button onClick={() => handleUpdateStatus(listing.id, listing.title, 'sold')} className="w-full text-left px-4 py-2 text-[#0D0D0D] hover:bg-black/[0.03] flex items-center gap-2">
