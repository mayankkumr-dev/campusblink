        ...
        <div className="flex flex-wrap items-center gap-2">
          {['All', 'Active', 'Suspended'].map(pill => (
            <button 
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-bold font-sans transition-colors ${filterStatus === pill ? 'bg-[#FFD600] text-[#0D0D0D]' : 'bg-[#F7F5F0] text-[#6B6B6B] hover:text-[#0D0D0D] border border-black/[0.08] hover:border-black/10'}`}
            >
              {pill}
            </button>
          ))}
          ...
          <button 
            ...
            className="flex items-center gap-2 px-4 py-2 bg-[#FFD600] hover:bg-yellow-400 text-[#0D0D0D] rounded-lg text-sm font-sans font-bold transition-colors"
          >
        ...
        <table className="w-full text-left border-collapse">
          <thead className="bg-[#F5F4F0] h-[40px] border-b border-[#E8E8E8]">
            <tr className="border-b border-black/[0.08] bg-[#F7F5F0] hover:bg-[#FAFAF8] transition-colors duration-150">
              <th className="p-4 text-xs font-bold text-[#6B6B6B] uppercase tracking-wider font-sans px-4 text-left font-sans font-semibold text-[12px] text-[#9B9B9B] uppercase tracking-[0.6px]">Shop</th>
              <th className="p-4 text-xs font-bold text-[#6B6B6B] uppercase tracking-wider font-sans px-4 text-left font-sans font-semibold text-[12px] text-[#9B9B9B] uppercase tracking-[0.6px]">Owner Info</th>
              <th className="p-4 text-xs font-bold text-[#6B6B6B] uppercase tracking-wider font-sans px-4 text-left font-sans font-semibold text-[12px] text-[#9B9B9B] uppercase tracking-[0.6px]">Orders Total</th>
              ...
              <tr key={shop.id} className="hover:bg-black/[0.03] transition-colors group">
                <td className="p-4 min-w-[200px]">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-lg bg-[#222222] flex items-center justify-center font-syne font-bold text-lg text-[#FFD600] border border-black/[0.08] shrink-0 shadow-inner overflow-hidden">
                      {shop.logo_url ? (
                    ...
                    <div>
                      <div className="font-sans font-bold text-sm text-[#0D0D0D] mb-0.5">{shop.name}</div>
                      <div className="font-sans text-xs text-[#6B6B6B]">{shop.college}</div>
                    </div>
                  </div>
                </td>
                <td className="p-4">
                   <div className="font-sans text-sm text-[#0D0D0D] font-medium mb-0.5">{shop.owner?.name || 'Unknown'}</div>
                   <div className="font-sans text-xs text-[#6B6B6B]">{shop.owner?.email || 'No email associated'}</div>
                </td>
                <td className="p-4">
                ...
                <td className="p-4 text-center relative">
                  ...
                  {activeDropdown === shop.id && (
                    <div className="absolute right-[50px] top-4 w-56 bg-[#F7F5F0] border border-black/10 rounded-lg shadow-md z-20 py-1 font-sans text-sm overflow-hidden animate-in zoom-in-95 duration-100">
                      
                      <button onClick={() => { navigate(`/admin/print/${shop.id}`); setActiveDropdown(null); }} className="w-full text-left px-4 py-2 text-[#0D0D0D] hover:bg-black/[0.03] flex items-center gap-2">
                        <Eye className="w-4 h-4 text-[#6B6B6B]" /> Schedule & Status
                      </button>
                      <button onClick={() => handleOpenEdit(shop)} className="w-full text-left px-4 py-2 text-[#0D0D0D] hover:bg-black/[0.03] flex items-center gap-2">
                        <Edit3 className="w-4 h-4 text-[#6B6B6B]" /> Edit Details
                      </button>
                      <button onClick={() => { navigate(`/admin/print/orders?shop=${shop.id}`); setActiveDropdown(null); }} className="w-full text-left px-4 py-2 text-[#0D0D0D] hover:bg-black/[0.03] flex items-center gap-2">
                        <FileText className="w-4 h-4 text-[#FFD600]" /> Print Orders
                      </button>
                      <button className="w-full text-left px-4 py-2 text-[#0D0D0D] hover:bg-black/[0.03] flex items-center gap-2">
                      ...
                      <button className="w-full text-left px-4 py-2 text-[#0D0D0D] hover:bg-black/[0.03] flex items-center gap-2">
                        <Mail className="w-4 h-4 text-[#0057FF]" /> Contact Owner
                      </button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="bg-white border border-black/10 rounded-lg w-full max-w-2xl max-h-[90vh] overflow-y-auto relative shadow-md animate-in zoom-in-95 duration-200 hide-scrollbar">
            ...
            <div className="sticky top-0 bg-white/90  border-b border-black/[0.08] p-6 flex items-center justify-between z-10">
              <h2 className="font-syne font-bold text-xl text-[#0D0D0D]">{isEditMode ? 'Edit Print Shop' : 'Register Print Shop'}</h2>
              <button onClick={() => setIsModalOpen(false)} className="p-2 text-[#6B6B6B] hover:text-[#0D0D0D] bg-[#F7F5F0] rounded-md transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <form onSubmit={handleCreateShop} className="p-6 space-y-6">
              <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
                <div className="md:col-span-2">
                  <label className="mb-2 block text-sm font-bold text-[#0D0D0D]">Shop Name</label>
                  <input
                    value={form.name}
                    onChange={(e) => setForm((prev) => ({ ...prev, name: e.target.value }))}
                    placeholder="Campus Xerox Hub"
                    className="w-full rounded-lg border border-black/10 bg-[#F7F5F0] px-4 py-3 text-sm text-[#0D0D0D] outline-none focus:border-[#FFD600]/50"
                    required
                  />
                </div>

                <div>
                  <label className="mb-2 block text-sm font-bold text-[#0D0D0D]">Owner</label>
                  <select
                    value={form.owner_id}
                    onChange={(e) => {
                ...
                <div>
                  <label className="mb-2 block text-sm font-bold text-[#0D0D0D]">College</label>
                  <input
                    value={form.college}
                ...
                <div>
                  <label className="mb-2 block text-sm font-bold text-[#0D0D0D]">B/W Price Per Page</label>
                  <input
                    type="number"
                    min="0"
                    step="1"
                ...
                <div>
                  <label className="mb-2 block text-sm font-bold text-[#0D0D0D]">Color Price Per Page</label>
                  <input
                    type="number"
                    min="0"
                    step="1"
                ...
                <div>
                  <label className="mb-2 block text-sm font-bold text-[#0D0D0D]">Binding Charge</label>
                  <input
                    type="number"
                    min="0"
                    step="1"
                ...
                <div>
                  <label className="mb-2 block text-sm font-bold text-[#0D0D0D]">Logo URL</label>
                  <input
                    value={form.logo_url}
              ...
              <label className="flex items-center gap-3 rounded-lg border border-black/[0.08] bg-[#F7F5F0] px-4 py-3 text-sm font-bold text-[#0D0D0D]">
                <input
                  type="checkbox"
              ...
              <div className="pt-6 border-t border-black/[0.08] flex justify-end gap-3">
                <button type="button" onClick={() => setIsModalOpen(false)} className="px-5 py-2.5 rounded-lg text-[#0D0D0D] font-sans font-bold hover:bg-black/[0.03] transition-colors">
                  Cancel
                </button>
                <button type="submit" disabled={isSaving} className="inline-flex items-center gap-2 rounded-lg bg-[#FFD600] px-5 py-2.5 font-sans font-bold text-[#0D0D0D] hover:bg-yellow-400 disabled:opacity-60">
