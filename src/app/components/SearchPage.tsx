...
export const SearchPage: React.FC = () => {
  ...
  return (
    <div className="min-h-full bg-[#FAFAF8] pb-24">
      <div className="mx-auto max-w-[860px] px-4 py-6 md:px-6">
        ...
        <div className="mb-6">
          <form onSubmit={handleSubmit} className="relative mb-3">
            ...
            <input
              ...
              className="h-10 w-full rounded-[6px] border border-[#E8E8E8] bg-[#F5F4F0] pl-10 pr-4 text-[15px] font-sans text-[#0D0D0D] placeholder:text-[#9B9B9B] outline-none focus:border-[#0D0D0D] focus:bg-white transition-colors"
              autoFocus
            />
          </form>
          ...
          {activeQuery && (
            <>
              <h1 className="font-syne font-bold text-[24px] text-[#0D0D0D] tracking-tight">
                Search results for &apos;{activeQuery}&apos;
              </h1>
              {!isLoading && (
                <p className="mt-1 font-sans text-[14px] text-[#9B9B9B]">
        ...
        {/* Tabs */}
        {activeQuery && (
          <div className="mb-6 flex items-center gap-1 border-b border-[#E8E8E8]">
            {tabs.map((tab) => (
              <button
                key={tab.label}
              ...
              >
                ...
                {tab.count > 0 && (
                  <span className={`rounded-full px-1.5 py-0.5 text-[11px] font-semibold ${
                    activeTab === tab.label ? 'bg-[#0D0D0D] text-white' : 'bg-[#F0F0F0] text-[#6B6B6B]'
                  }`}>
                    {tab.count}
                  </span>
                )}
        ...
        {isLoading && (
          <div className="flex items-center justify-center py-20">
            <Loader2 className="h-7 w-7 animate-spin text-[#9B9B9B]" />
          </div>
        )}

        {/* No query state */}
        {!activeQuery && !isLoading && (
          <div className="flex flex-col items-center justify-center py-20 gap-3 text-[#9B9B9B]">
            <Search className="h-10 w-10 text-[#E8E8E8]" />
        ...
        {!isLoading && totalCount > 0 && (
          <div className="space-y-8">
            ...
            {showStudents && students.length > 0 && (
              <section>
                {activeTab === 'All' && (
                  <h2 className="mb-4 font-syne font-bold text-[18px] text-[#0D0D0D]">Students</h2>
                )}
                <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                  {students.map((student) => {
                    ...
                    return (
                      <div
                        key={student.id}
                        className="flex flex-col items-center rounded-[8px] border border-[#E8E8E8] bg-white p-5 text-center"
                      >
                        <button
                        ...
                        >
                          <img
                            src={avatar}
                            alt={student.name}
                            className="h-16 w-16 rounded-full border border-[#E8E8E8] object-cover mb-3"
                          />
                          <p className="font-syne font-semibold text-[16px] text-[#0D0D0D] leading-tight">
                            {student.name}
                          </p>
                          <p className="mt-0.5 font-sans text-[13px] text-[#9B9B9B]">
                            @{getDisplayHandle(student.username, 'student')}
                          </p>
                          {student.college && (
                            <span className="mt-2 rounded-md bg-[#FFD600]/15 px-2 py-0.5 text-[11px] font-bold uppercase tracking-[0.12em] text-[#0D0D0D]">
                              {student.college.includes('(MAIT)') ? 'MAIT' : student.college.split(' ').slice(-1)[0].replace(/[()]/g, '')}
                            </span>
                          )}
                          {student.campus_credits != null && (
                            <p className="mt-1.5 font-sans text-[12px] text-[#6B6B6B]">
                              ⭐ {student.campus_credits} Reputation
                            </p>
                          )}
                          {student.bio && (
                            <p className="mt-2 font-sans text-[13px] text-[#6B6B6B] line-clamp-2 leading-snug">
                              {student.bio}
                            </p>
                          )}
                        </button>
            ...
            {showPosts && posts.length > 0 && (
              <section>
                {activeTab === 'All' && (
                  <h2 className="mb-4 font-syne font-bold text-[18px] text-[#0D0D0D]">Posts</h2>
                )}
                <div className="rounded-[16px] border border-black/10 bg-white overflow-hidden">
                  {posts.map((post, i) => {
                    const avatar =
                    ...
                    return (
                      <article
                        ...
                        className={`cursor-pointer px-4 py-4 hover:bg-[#FAFAF8] transition-colors ${i > 0 ? 'border-t border-black/10' : ''}`}
                      >
                        <div className="flex gap-3">
                          ...
                          <div className="min-w-0 flex-1">
                            <div className="flex items-center gap-2 flex-wrap">
                              <span className="font-sans font-semibold text-[13px] text-[#0D0D0D]">
                                {post.author?.name || 'Campus Student'}
                              </span>
                              {post.type && (
                                <span className="rounded-md bg-[#F5F4F0] px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-[0.10em] text-[#6B6B6B]">
                                  {post.type}
                                </span>
                              )}
                            ...
                            {post.title && (
                              <h3 className="mt-1 font-sans font-semibold text-[14px] text-[#0D0D0D] leading-snug">
                                {post.title}
                              </h3>
                            )}
                            <p className="mt-0.5 font-sans text-[13px] text-[#6B6B6B] line-clamp-2 leading-snug">
                              {post.content}
                            </p>
                            {thumbnail && (
                              <div className="mt-2 overflow-hidden rounded-[8px] border border-black/10 bg-[#F2F0EB]">
                                <img src={thumbnail} alt="Post" className="max-h-[160px] w-full object-cover" />
                              </div>
                            )}
                            <div className="mt-2 flex items-center gap-4 text-[12px] text-[#9B9B9B]">
            ...
            {showListings && listings.length > 0 && (
              <section>
                {activeTab === 'All' && (
                  <h2 className="mb-4 font-syne font-bold text-[18px] text-[#0D0D0D]">Listings</h2>
                )}
                  ...
                  {listings.map((listing) => {
                    ...
                    return (
                      <div
                        ...
                        className="cursor-pointer rounded-[8px] border border-[#E8E8E8] bg-white overflow-hidden hover:shadow-md transition-shadow"
                      >
                        <div className="aspect-square bg-[#F5F4F0]">
                          {thumbnail ? (
                            <img src={thumbnail} alt={listing.title} className="h-full w-full object-cover" />
                          ) : (
                            <div className="flex h-full w-full items-center justify-center text-[#9B9B9B] font-sans text-[11px]">
                              No image
                            </div>
                          )}
                        </div>
                        <div className="p-3">
                          <p className="font-sans font-semibold text-[13px] text-[#0D0D0D] line-clamp-2 leading-snug">
                            {listing.title}
                          </p>
                          <div className="mt-1.5 flex items-center gap-2">
                            <span className="font-syne font-semibold text-[13px] text-[#0D0D0D]">
