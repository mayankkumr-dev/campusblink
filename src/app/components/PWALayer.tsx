...
export const PWALayer: React.FC = () => {
  ...
  return (
    <>
      {isOffline && (
        <div className="fixed top-3 left-1/2 -translate-x-1/2 z-[120] rounded-md bg-black/90 px-4 py-2 text-xs font-bold text-[#FFD600] shadow-strong">
          You are offline
        </div>
      )}
      ...
      {showUpdate && (
        <button
          ...
          className="fixed top-12 left-1/2 -translate-x-1/2 z-[120] rounded-md bg-[#FFD600] px-5 py-2 text-xs font-bold text-[#0D0D0D] shadow-strong"
        >
      ...
      {showInstallBanner && !inStandalone && deferredPrompt && (
        <div className="fixed bottom-4 left-4 right-4 z-[120] rounded-lg border border-black/10 bg-white/95 p-4 shadow-strong ">
          <div className="flex items-center gap-3">
            <img src="/logo/only_logo.png" alt="Campus Blink" className="h-10 w-10 rounded-lg" />
            <div className="min-w-0 flex-1">
              <p className="truncate text-sm font-extrabold text-[#0D0D0D]">Install Campus Blink</p>
              <p className="truncate text-xs text-[#6B6B6B]">Works offline · Faster · No browser bar</p>
            </div>
            <button onClick={handleInstall} className="rounded-lg bg-[#FFD600] px-3 py-2 text-xs font-bold text-[#0D0D0D]">
              Install
            </button>
            <button onClick={dismissInstallForSevenDays} className="rounded-lg p-2 text-[#6B6B6B]" aria-label="Dismiss install banner">
              <X className="h-4 w-4" />
            </button>
          </div>
        </div>
      ...
      {showIOSBanner && !inStandalone && (
        <div className="fixed bottom-4 left-4 right-4 z-[120] rounded-lg border border-black/10 bg-white p-4 shadow-strong">
          <div className="flex items-start gap-3">
            <Download className="mt-1 h-5 w-5 text-[#0D0D0D]" />
            <div className="min-w-0 flex-1">
              <p className="text-sm font-bold text-[#0D0D0D]">Install on iPhone</p>
              <p className="text-xs text-[#6B6B6B]">Install on iPhone: tap Share button then Add to Home Screen</p>
            </div>
            <button onClick={() => setShowIOSBanner(false)} className="rounded-lg p-2 text-[#6B6B6B]" aria-label="Dismiss iOS install banner">
              <X className="h-4 w-4" />
            </button>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-[130] flex items-end justify-center bg-black/35 p-4 md:items-center">
          <div className="w-full max-w-md rounded-lg bg-white p-5 shadow-strong">
            <div className="mb-3 flex items-center gap-2">
              ...
              <h3 className="text-base font-extrabold text-[#0D0D0D]">Get notified when your order is ready?</h3>
            </div>
            <p className="mb-5 text-sm text-[#6B6B6B]">Allow notifications so Campus Blink can alert you when your status changes.</p>
            <div className="flex items-center gap-3">
              <button onClick={handleEnableNotifications} className="flex-1 rounded-lg bg-[#FFD600] px-4 py-2.5 text-sm font-bold text-[#0D0D0D]">
                Yes
              </button>
              <button onClick={() => setShowNotificationPrompt(false)} className="flex-1 rounded-lg border border-black/10 px-4 py-2.5 text-sm font-bold text-[#0D0D0D]">
                No thanks
              </button>
            </div>
          </div>
