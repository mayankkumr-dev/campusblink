...
export const LoginRegisterPage: React.FC = () => {
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    if (tab === 'register') {
      ...
      const { error } = await signUp(email, password, fullName, college, normalizedUsername, {
    ...
    if (role === 'admin' || isAdminEmail) navigate('/admin');
  ...
  const handleForgotPassword = async () => {
    if (!email) {
      toast.error('Enter your email first to reset the password.');
      return;
    }
  ...
  if (showPostSignupScreen && pendingVerification) {
    return (
      <div className="min-h-screen bg-[#FAFAF8] p-3 md:p-4 flex text-[#0D0D0D] font-sans overflow-hidden">
        <div className="flex-1 flex flex-col justify-between p-6 md:p-12 relative z-10">
          <div className="flex justify-center mb-8">
            <Link to="/" className="flex flex-col items-center justify-center drop-shadow-sm transition-transform hover:scale-105">
              <img src={appLogoDiamond} alt="Campus Blink Icon" loading="eager" className="h-[65px] w-auto object-contain shrink-0" />
              <img src={textTransparent} alt="Campus Blink" loading="eager" className="h-[85px] w-auto object-contain -mt-5 shrink-0" />
            </Link>
          </div>
          ...
          <div className="max-w-xl mx-auto w-full flex-1 flex flex-col items-center justify-center text-center py-8">
            <motion.div
              ...
              className="w-20 h-20 rounded-md bg-[#FFD600]/25 border border-[#FFD600]/50 flex items-center justify-center mb-6"
            >
              <Mail className="w-10 h-10 text-[#0D0D0D]" />
            </motion.div>

            <h1 className="font-syne font-extrabold text-4xl md:text-5xl text-[#0D0D0D] mb-4 leading-tight">
              Check your inbox, {pendingVerification.firstName}! 📬
            </h1>
            <p className="font-sans text-[#6B6B6B] text-base md:text-lg max-w-lg leading-relaxed">
              We sent a verification link to <span className="font-bold text-[#0D0D0D]">{pendingVerification.email}</span>.
            ...
            <div className="mt-8 w-full max-w-sm space-y-3">
              <button
                ...
                className="w-full rounded-lg border border-[#0D0D0D]/20 bg-white px-4 py-3 text-sm font-bold text-[#0D0D0D] hover:bg-[#0D0D0D]/5 transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
              >
              ...
              <button
                ...
                className="w-full text-sm font-bold text-[#6B6B6B] hover:text-[#0D0D0D]"
              >
  ...
  return (
    <div className="min-h-screen lg:h-screen bg-[linear-gradient(180deg,#F7F4EA_0%,#FAFAF8_100%)] p-2 md:p-3 lg:p-4 flex text-[#0D0D0D] font-sans overflow-hidden">
      <img
        src={appLogoDiamond}
        alt="Campus Blink" loading="lazy" width={800} height={800} style={{ objectFit: 'contain' }}
        className="fixed inset-0 w-full h-full opacity-[0.01] pointer-events-none mix-blend-screen scale-150 lg:hidden"
      />

      <div className="relative hidden md:flex md:w-[46%] lg:w-1/2 overflow-hidden rounded-[2rem] border border-white/5 bg-[#0A0A0A] justify-center items-center">
        
        {/* Mascot Watermark */}
        <img
          src={appLogoDiamond}
          alt=""
          className="absolute inset-0 m-auto w-[160%] h-[160%] opacity-[0.04] object-contain rotate-[-10deg] pointer-events-none"
        />
        ...
        <div className="absolute inset-0 z-10 w-full h-full pointer-events-none">
            ...
            <motion.div
              ...
              className="absolute top-[20%] left-[8%] -rotate-3 p-4 rounded-xl bg-white/[0.04] backdrop-blur-md border border-white/10 shadow-[0_0_30px_rgba(255,214,0,0.08)] min-w-[220px]"
            >
               <div className="flex items-center justify-between gap-4 mb-2">
                  <span className="text-white font-syne font-medium text-[15px]">🍔 Vada Pav ordered!</span>
                  <span className="px-2 py-0.5 rounded-full bg-[#10B981]/20 text-[#10B981] text-[10px] font-bold tracking-wide">Ready</span>
               </div>
               <p className="text-white/60 text-[13px] font-sans">Ready at 1:00 PM slot</p>
            </motion.div>
            ...
            <motion.div
              ...
              className="absolute top-[48%] left-[2%] rotate-2 p-4 rounded-xl bg-white/[0.04] backdrop-blur-md border border-white/10 shadow-[0_0_30px_rgba(255,214,0,0.08)] min-w-[220px]"
            >
               <div className="flex items-center justify-between gap-4 mb-2">
                  <span className="text-white font-syne font-medium text-[15px]">📚 HC Verma Physics</span>
                  <span className="px-2 py-0.5 rounded-full bg-[#FFD600]/20 text-[#FFD600] text-[10px] font-bold tracking-wide">₹180</span>
               </div>
               <p className="text-white/60 text-[13px] font-sans">Like New</p>
            </motion.div>
            ...
            <motion.div
              ...
              className="absolute bottom-[20%] right-[8%] -rotate-1 p-4 rounded-xl bg-white/[0.04] backdrop-blur-md border border-white/10 shadow-[0_0_30px_rgba(255,214,0,0.08)] min-w-[220px]"
            >
               <div className="flex items-center gap-2 mb-2">
                  <span className="text-white font-syne font-medium text-[15px]">⭐ +20 Reputation</span>
               </div>
               <p className="text-white/60 text-[13px] font-sans">Friend joined via your invite!</p>
            </motion.div>
            ...
            <motion.div
              ...
              className="absolute top-[18%] right-[5%] rotate-4 p-4 rounded-xl bg-white/[0.04] backdrop-blur-md border border-white/10 shadow-[0_0_30px_rgba(255,214,0,0.08)] min-w-[220px]"
            >
               <div className="flex items-center gap-2 mb-2">
                  <span className="text-white font-syne font-medium text-[15px]">💬 Anonymous confession</span>
               </div>
               <p className="text-white/60 text-[13px] font-sans">47 likes · 12 replies</p>
            </motion.div>
        </div>

        {/* Center Overlay Text */}
        <div className="relative z-20 flex flex-col items-center text-center -mt-8">
            <h2 className="font-syne font-extrabold text-[#ffffff] text-[52px] leading-[1.1] tracking-[-1px] drop-shadow-2xl">
                Stop waiting.<br />
                Start <span className="text-[#FFD600]">blinking.</span>
            </h2>
        </div>
      </div>

      <div className="flex-1 flex flex-col justify-between p-5 md:p-8 lg:px-9 lg:py-7 relative z-10 overflow-y-auto lg:overflow-hidden">
        <div className="flex justify-center mb-4 lg:mb-3">
          <Link to="/" className="flex flex-col items-center justify-center drop-shadow-sm transition-transform hover:scale-105">
            <img src={appLogoDiamond} alt="Campus Blink Icon" loading="eager" className="h-[52px] w-auto object-contain shrink-0" />
            <img src={textTransparent} alt="Campus Blink" loading="eager" className="h-[68px] w-auto object-contain -mt-4 shrink-0" />
          </Link>
        </div>

        <div className="max-w-md mx-auto w-full flex-1 flex flex-col justify-center py-4 lg:py-2 rounded-[28px] border border-black/10 bg-white/92 px-5 shadow-[0_24px_60px_rgba(13,13,13,0.08)] backdrop-blur-sm md:px-7">
          <AnimatePresence mode="wait">
            ...
            >
              <div className="text-center mb-7 lg:mb-6">
                <h2 className="text-[2rem] lg:text-[2.2rem] font-syne font-bold text-[#0D0D0D] mb-2">
                  {tab === 'login' ? 'Welcome Back' : registerStep === 1 ? 'You need an invite.' : 'Join Campus'}
                </h2>
                <p className="text-[#6B6B6B] font-sans text-sm lg:text-[13px]">
                  {tab === 'login'
                ...
                {tab === 'register' && registerStep === 2 ? (
                  <p className="mt-2 text-xs font-bold uppercase tracking-[0.16em] text-[#CA8A04]">You received 50 Reputation Points as a welcome bonus!</p>
                ) : null}
              </div>

              {emailVerifiedBanner && (
                <div className="mb-5 rounded-lg border border-[#00E676]/40 bg-[#00E676]/10 px-4 py-3">
                  <p className="font-sans font-bold text-sm text-[#0D0D0D]">✅ Email verified! You can now login.</p>
                </div>
              )}

              {verifyingEmailLink && (
                <div className="mb-5 rounded-lg border border-[#FFD600]/40 bg-[#FFD600]/10 px-4 py-3 flex items-center gap-2">
                  <Loader2 className="w-4 h-4 animate-spin text-[#0D0D0D]" />
                  <p className="font-sans font-bold text-sm text-[#0D0D0D]">Verifying your email link...</p>
                </div>
              )}
              ...
              {tab === 'register' && registerStep === 1 ? (
                <div className="space-y-4">
                  <div className="text-center">
                    ...
                    <p className="font-sans text-sm text-[#6B6B6B]">Enter your invite code to continue</p>
                  </div>
                  ...
                  >
                    ...
                    <Input
                      ...
                      className={`uppercase tracking-[0.18em] text-center font-bold ${inviteError ? 'border-[#FF3D57] focus:border-[#FF3D57] focus:ring-[#FF3D57]/25' : inviteValidated ? 'border-[#16A34A] focus:border-[#16A34A] focus:ring-[#16A34A]/25' : ''}`}
                      required
                    />
                  </motion.div>
                  ...
                  {inviteValidated?.inviter?.name ? (
                    <div className="rounded-lg border border-[#16A34A]/30 bg-[#16A34A]/10 px-4 py-3">
                      <p className="font-sans text-sm font-bold text-[#0D0D0D]">✅ Invited by {getFirstName(inviteValidated.inviter.name, 'a friend')}</p>
                    </div>
                  ) : null}

                  {inviteError ? (
                    <div className="rounded-lg border border-[#FF3D57]/30 bg-[#FF3D57]/10 px-4 py-3">
                  ...
                  <Button
                    ...
                    className="w-full bg-[#FFD600] text-[#0D0D0D] font-bold hover:bg-[#0D0D0D] hover:text-[#FFD600]"
                  >
              ...
              <form onSubmit={handleAuth} className="space-y-4 lg:space-y-3.5">
                {tab === 'register' && inviteValidated ? (
                  <div className="rounded-lg border border-[#16A34A]/30 bg-[#16A34A]/10 px-4 py-3 flex items-center justify-between gap-3">
                    ...
                    <span className="text-xs font-bold text-[#166534] uppercase tracking-[0.16em]">{inviteValidated.code}</span>
                  </div>
                ) : null}
                {tab === 'register' && (
                  <motion.div
                  ...
                  >
                      ...
                      <Input
                        ...
                        className={usernameStatus.state === 'available'
                          ...
                          : usernameStatus.state === 'unavailable'
                            ? 'border-[#FF3D57] focus:border-[#FF3D57] focus:ring-[#FF3D57]/25'
                            : ''}
                      />
                      <p className="mt-2 text-xs font-medium text-[#6B6B6B]">Only lowercase letters, numbers, dots, and underscores. 3-20 characters.</p>
                      {usernameStatus.state !== 'idle' ? (
                        <p className={`mt-1 text-xs font-bold ${usernameStatus.state === 'available' ? 'text-[#166534]' : usernameStatus.state === 'checking' ? 'text-[#6B6B6B]' : 'text-[#B42318]'}`}>
                    ...
                    <div>
                      ...
                      <select
                        ...
                        required
                        className="w-full h-12 rounded-lg border border-[#E8E8E8] transition-all appearance-none bg-[linear-gradient(45deg,transparent_50%,#0D0D0D_50%),linear-gradient(135deg,#0D0D0D_50%,transparent_50%)] bg-[position:calc(100%-18px)_22px,calc(100%-12px)_22px] bg-[size:6px_6px,6px_6px] bg-no-repeat"
                      >
                ...
                <div className="relative">
                  ...
                  <button
                    ...
                    className="absolute right-4 bottom-1.5 text-[#AAAAAA] hover:text-[#0D0D0D] transition-colors"
                  >
                ...
                {tab === 'login' && (
                  <div className="flex items-center justify-between pt-2">
                    <label className="flex items-center gap-2 cursor-pointer group">
                      <input
                        type="checkbox"
                        className="w-4 h-4 rounded border-black/20 bg-white text-[#FFD600] focus:ring-1 focus:ring-[#FFD600] focus:ring-offset-0 focus:ring-offset-transparent cursor-pointer transition-colors accent-[#FFD600]"
                      />
                      <span className="text-sm text-[#6B6B6B] group-hover:text-[#0D0D0D] transition-colors">Remember me</span>
                    </label>
                    <button type="button" onClick={handleForgotPassword} className="text-sm font-medium text-[#6B6B6B] hover:text-[#FFD600] transition-colors">
                      Forgot Password?
                    </button>
                  </div>
                )}
                  ...
                  <div
                    className={`rounded-lg border px-4 py-3 ${
                      authStatus.type === 'error'
                        ? 'border-[#FF3D57]/30 bg-[#FF3D57]/10'
                        : authStatus.type === 'success'
                          ? 'border-[#00E676]/30 bg-[#00E676]/10'
                          : 'border-[#FFD600]/40 bg-[#FFD600]/10'
                    }`}
                  >
                    <p className="font-sans font-bold text-sm text-[#0D0D0D]">{authStatus.title}</p>
                    <p className="font-sans text-sm text-[#6B6B6B] mt-1">{authStatus.message}</p>
                  </div>
                )}
                ...
                {tab === 'login' && showResendConfirmation && (
                  <button
                    ...
                    onClick={handleResendConfirmation}
                    disabled={resendCooldown > 0}
                    className="w-full rounded-lg border border-[#FFD600]/40 bg-[#FFD600]/10 px-4 py-3 text-sm font-bold text-[#0D0D0D] hover:bg-[#FFD600]/20 transition-colors disabled:opacity-60"
                  >
                    {resendCooldown > 0 ? `Resend in ${resendCooldown}s...` : 'Resend Verification Email'}
                  </button>
                )}
                ...
                <div className="pt-4 space-y-3">
                  <Button disabled={isLoading || (tab === 'register' && usernameStatus.state !== 'available')} type="submit" size="lg" className="w-full bg-[#0D0D0D] text-white font-bold hover:bg-[#FFD600] hover:text-[#0D0D0D]">
                  ...
                  {tab === 'login' && (
                    <Button
                      ...
                      variant="ghost"
                      size="lg"
                      className="w-full border border-[#0D0D0D] text-[#0D0D0D] hover:bg-[#0D0D0D] hover:text-white flex items-center justify-center gap-3 font-sans font-medium capitalize-none normal-case tracking-normal"
                    >
        ...
        <div className="text-center mt-auto pt-4 pb-1 lg:pt-3 lg:pb-0">
          <p className="text-sm text-[#6B6B6B] font-sans">
            ...
            <button
              ...
              className="font-bold text-[#0D0D0D] hover:text-[#FFD600] transition-colors"
            >
