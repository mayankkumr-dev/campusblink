        ...
        <table className="w-full text-left border-collapse">
          <thead className="bg-[#F5F4F0] h-[40px] border-b border-[#E8E8E8]">
            <tr className="border-b border-black/[0.08] bg-[#F7F5F0] hover:bg-[#FAFAF8] transition-colors duration-150">
              <th className="p-4 text-xs font-bold text-[#6B6B6B] uppercase tracking-wider font-sans px-4 text-left font-sans font-semibold text-[12px] text-[#9B9B9B] uppercase tracking-[0.6px]">Canteen</th>
              ...
              <th className="p-4 text-xs font-bold text-[#6B6B6B] uppercase tracking-wider font-sans px-4 text-left font-sans font-semibold text-[12px] text-[#9B9B9B] uppercase tracking-[0.6px]">Performance</th>
              ...
              <tr key={canteen.id} className="hover:bg-black/[0.03] transition-colors group">
                <td className="p-4 min-w-[200px]">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-lg bg-[#222222] flex items-center justify-center font-syne font-bold text-lg text-[#FFD600] border border-black/[0.08] shrink-0 overflow-hidden">
                      {canteen.logo_url ? (
                    ...
                    <div>
                      <div className="font-sans font-bold text-sm text-[#0D0D0D] mb-0.5">{canteen.name}</div>
                      <div className="font-sans text-xs text-[#6B6B6B]">{canteen.college}</div>
                    </div>
                  </div>
                </td>
                <td className="p-4">
                   <div className="font-sans text-sm text-[#0D0D0D] font-medium mb-0.5">{canteen.owner?.name || 'Unknown'}</div>
                   <div className="font-sans text-xs text-[#6B6B6B]">{canteen.owner?.email || 'No email associated'}</div>
                </td>
                <td className="p-4">
                  <div className="font-sans text-sm text-[#0D0D0D] flex gap-2 mb-0.5">
                     <span className="text-[#6B6B6B]">Orders:</span> {canteen.total_orders || 0}
                  </div>
                ...
                <td className="p-4 text-center relative">
                  ...
                  {activeDropdown === canteen.id && (
                    <div className="absolute right-[50px] top-4 w-56 bg-[#F7F5F0] border border-black/10 rounded-lg shadow-md z-20 py-1 font-sans text-sm overflow-hidden animate-in zoom-in-95 duration-100">
                      ...
                      <button onClick={() => { navigate(`/admin/canteen/${canteen.id}`); setActiveDropdown(null); }} className="w-full text-left px-4 py-2 text-[#0D0D0D] hover:bg-black/[0.03] flex items-center gap-2">
                        <Eye className="w-4 h-4 text-[#6B6B6B]" /> Schedule & Status
                      </button>
                      <button onClick={() => handleOpenEdit(canteen)} className="w-full text-left px-4 py-2 text-[#0D0D0D] hover:bg-black/[0.03] flex items-center gap-2">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="bg-white border border-black/10 rounded-lg w-full max-w-2xl max-h-[90vh] overflow-y-auto relative shadow-md animate-in zoom-in-95 duration-200 hide-scrollbar">
            ...
            <div className="sticky top-0 bg-white/90  border-b border-black/[0.08] p-6 flex items-center justify-between z-10">
              <h2 className="font-syne font-bold text-xl text-[#0D0D0D]">{isEditMode ? 'Edit Canteen' : 'Register New Canteen'}</h2>
            ...
            <form onSubmit={handleSaveCanteen} className="p-6 space-y-6">
              <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
                <div className="md:col-span-2">
                  <label className="mb-2 block text-sm font-bold text-[#0D0D0D]">Canteen Name</label>
                  <input
                    value={form.name}
