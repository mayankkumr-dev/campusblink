  ...
  return (
    <>
      ...
      <div
        ...
        aria-modal="true"
        aria-label="Search"
        className={`fixed left-0 top-0 h-full z-[76] flex flex-col bg-[#FAFAF8] border-r border-black/[0.08] shadow-[6px_0_40px_rgba(0,0,0,0.14)] transition-transform duration-300 ease-in-out w-full md:w-[390px] ${
          isOpen ? 'translate-x-0' : '-translate-x-full'
        }`}
      >
        {/* Header */}
        <div className="flex h-[70px] shrink-0 items-center justify-between px-6 border-b border-black/[0.06]">
          <h2 className="font-syne font-extrabold text-[22px] text-[#0D0D0D] tracking-tight">Search</h2>
          <button
            type="button"
            onClick={onClose}
            className="flex h-9 w-9 items-center justify-center rounded-full text-[#6B6B6B] hover:bg-black/[0.06] hover:text-[#0D0D0D] transition-colors"
        ...
        <div className="px-5 pt-4 pb-3 shrink-0">
          <form onSubmit={handleSubmit} className="relative">
            <Search
              size={16}
              className="pointer-events-none absolute left-3.5 top-1/2 -translate-y-1/2 text-[#9B9B9B]"
            />
            <input
              ref={inputRef}
              type="text"
              ...
              className="h-[44px] w-full rounded-[10px] border border-black/[0.08] bg-[#F2F0EB] pl-10 pr-10 text-[14px] font-sans text-[#0D0D0D] placeholder:text-[#ACACAC] outline-none focus:border-[#FFD600] focus:bg-white transition-colors"
            />
            {query && (
              <button
                type="button"
                onClick={handleClearQuery}
                className="absolute right-3 top-1/2 -translate-y-1/2 flex h-[22px] w-[22px] items-center justify-center rounded-full bg-[#C8C6BE] text-white hover:bg-[#9B9B9B] transition-colors"
        ...
        <div className="flex-1 overflow-y-auto">
            ...
            <div>
              <div className="flex items-center justify-between px-5 pt-2 pb-1">
                ...
                <button
                  ...
                  className="font-sans text-[13px] font-semibold text-[#6B6B6B] hover:text-[#0D0D0D] transition-colors"
                >
                  Clear all
                </button>
              </div>
              {recentSearches.map((term) => (
                ...
                >
                  ...
                  <span className="flex-1 font-sans text-[14px] font-medium text-[#0D0D0D] truncate">{term}</span>
                  <button
                    type="button"
                    onClick={(e) => handleRemoveRecent(e, term)}
                    className="opacity-0 group-hover:opacity-100 flex h-7 w-7 items-center justify-center rounded-full text-[#9B9B9B] hover:bg-black/[0.06] hover:text-[#0D0D0D] transition-all"
          ...
          {!query.trim() && !showRecent && (
            <div className="flex flex-col items-center justify-center py-24 gap-3">
              ...
              <p className="font-sans text-[14px] text-[#9B9B9B]">Search for students, posts or listings</p>
            </div>
          )}
          ...
          {showEmpty && (
            <div className="flex flex-col items-center justify-center py-16 gap-2 px-8 text-center">
              ...
              <p className="font-sans text-[14px] text-[#9B9B9B]">No results for &apos;{query}&apos;</p>
            </div>
          )}
            ...
            <>
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="min-w-0 flex-1">
                          <p className="truncate font-sans font-semibold text-[14px] text-[#0D0D0D] leading-tight">
                            {student.name}
                          </p>
                          <p className="truncate font-sans text-[12px] text-[#9B9B9B] leading-tight">
                  ...
                  {results.posts.map((post) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        />
                        <div className="min-w-0 flex-1">
                          <p className="font-sans font-semibold text-[13px] text-[#0D0D0D] truncate leading-tight">
                            {post.author?.name || 'Campus Student'}
                          </p>
                          <p className="font-sans text-[12px] text-[#6B6B6B] line-clamp-2 leading-snug">
                  ...
                  {results.listings.map((listing) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="min-w-0 flex-1">
                          <p className="truncate font-sans font-semibold text-[13px] text-[#0D0D0D] leading-tight">
                            {listing.title}
                          </p>
                          <div className="flex items-center gap-2 mt-0.5">
                            ...
                            {listing.category && (
                              <span className="rounded-md bg-[#F2F0EB] px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-[0.10em] text-[#6B6B6B]">
                                {listing.category}
                              </span>
                            )}
                          </div>
              ...
              <div className="border-t border-black/[0.05] px-5 py-3 mt-1">
                <button
                  ...
                  className="w-full flex items-center justify-center gap-1.5 rounded-[8px] py-2.5 text-[13px] font-semibold text-[#6B6B6B] hover:bg-black/[0.04] hover:text-[#0D0D0D] transition-colors"
                >
