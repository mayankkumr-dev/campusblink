...
export function MarketplaceMessagesPage() {
  ...
  return (
    <div className="min-h-screen bg-[linear-gradient(180deg,#FAFAF8_0%,#F0EBDF_100%)] px-4 py-5 sm:px-6 lg:px-8">
      <div className="w-full space-y-6">
        <div className="flex flex-wrap items-center justify-between gap-3 rounded-[28px] border border-black/8 bg-white px-5 py-4 shadow-[0_20px_70px_rgba(0,0,0,0.06)]">
          <div>
            <div className="text-xs font-semibold uppercase tracking-[0.24em] text-[#7C6C20]">Marketplace inbox</div>
            <h1 className="mt-1 text-2xl font-black tracking-tight text-[#0D0D0D]">Messages and offers</h1>
          </div>
          <div className="flex flex-wrap gap-2">
            <Link to="/student/buy-sell" className="rounded-md border border-black/10 px-4 py-2 text-sm font-semibold text-[#0D0D0D]">
              Browse listings
            </Link>
            <Link to="/student/wishlist" className="rounded-md border border-black/10 px-4 py-2 text-sm font-semibold text-[#0D0D0D]">
              Wishlist
            </Link>
          </div>
        </div>
        ...
        <div className="grid gap-6 lg:grid-cols-[340px_1fr]">
          <section className="overflow-hidden rounded-[32px] border border-black/8 bg-white shadow-[0_24px_80px_rgba(0,0,0,0.08)]">
            <div className="border-b border-black/8 px-5 py-4">
              <div className="text-xs font-semibold uppercase tracking-[0.22em] text-[#7C6C20]">All conversations</div>
              <div className="mt-1 text-lg font-black tracking-tight text-[#0D0D0D]">{conversations.length} active threads</div>
            </div>
              ...
              {isLoading ? (
                <div className="space-y-3 p-4">
                  {new Array(5).fill(null).map((_, index) => (
                    <div key={index} className="rounded-[24px] bg-[#F4EFE2] p-4 animate-pulse">
                      <div className="h-4 w-1/2 rounded-md bg-white/80" />
                      <div className="mt-3 h-3 w-4/5 rounded-md bg-white/80" />
                    </div>
                  ))}
                </div>
                  ...
                  {conversations.map((conversation) => {
                    ...
                    return (
                      <button
                        ...
                        className={`w-full rounded-[24px] border p-4 text-left transition ${isActive ? 'border-[#FFD600] bg-[#FFFCF2]' : 'border-transparent bg-white hover:bg-[#FAF7EF]'}`}
                      >
                        <div className="flex items-center gap-3">
                          <MarketplaceAvatar profile={otherProfile} size="sm" />
                          <div className="min-w-0 flex-1">
                            <div className="flex items-center justify-between gap-2">
                              <div className="truncate text-sm font-bold text-[#0D0D0D]">{getProfileName(otherProfile)}</div>
                              <div className="text-[11px] text-[#8A8577]">{formatMarketplaceTime(conversation.last_message_at)}</div>
                            </div>
                            <div className="mt-1 truncate text-xs text-[#6B675E]">{conversation.listing?.title || 'Marketplace listing'}</div>
                            <div className="mt-2 flex items-center justify-between gap-3">
                              <div className="truncate text-sm text-[#4F4A40]">{conversation.last_message || 'Say hi to start the deal.'}</div>
                              {unreadCount ? (
                                <div className="inline-flex min-w-6 items-center justify-center rounded-md bg-[#FFD600] px-2 py-1 text-[10px] font-black text-[#0D0D0D]">
                                  {unreadCount}
                                </div>
                              ) : null}
                            </div>
              ...
              <>
                <div className="flex flex-wrap items-center justify-between gap-4 border-b border-black/8 px-5 py-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="text-lg font-black tracking-tight text-[#0D0D0D]">{getProfileName(peerProfile)}</div>
                      <div className="text-sm text-[#6A665C]">{activeConversation.listing?.title || 'Marketplace listing'}</div>
                    </div>
                  </div>
                  <Link to={`/student/buy-sell/${activeConversation.listing_id}`} className="rounded-md border border-black/10 px-4 py-2 text-sm font-semibold text-[#0D0D0D]">
                    View listing
                  </Link>
                </div>

                <div className="border-b border-black/8 bg-[#FFFCF5] px-5 py-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="truncate text-sm font-bold text-[#0D0D0D]">{activeConversation.listing?.title}</div>
                      <div className="mt-1 text-sm text-[#6A665C]">{formatPrice(activeConversation.listing?.price)}</div>
                    </div>
                  </div>
                </div>

                <div className="max-h-[58vh] space-y-4 overflow-y-auto bg-[linear-gradient(180deg,#FFFDF8_0%,#F7F2E7_100%)] px-4 py-5 sm:px-5">
                  {isLoadingMessages ? (
                    <div className="space-y-3">
                      {new Array(4).fill(null).map((_, index) => (
                        <div key={index} className="h-16 w-3/4 animate-pulse rounded-[24px] bg-white/80" />
                      ))}
                    </div>
                    ...
                    return (
                      <div key={message.id} className={`flex ${isOwn ? 'justify-end' : 'justify-start'}`}>
                        <div className={`max-w-[86%] rounded-[28px] px-4 py-3 shadow-sm ${isOwn ? 'bg-[#0D0D0D] text-white' : 'bg-white text-[#0D0D0D]'}`}>
                          ...
                          {message.message_type === 'offer' ? (
                            <div>
                              <div className={`inline-flex items-center gap-2 rounded-md px-3 py-1 text-xs font-semibold uppercase tracking-[0.18em] ${isOwn ? 'bg-white/12 text-[#FFD600]' : 'bg-[#FFF2B0] text-[#6A5600]'}`}>
                                <IndianRupee className="h-3.5 w-3.5" />
                                Offer
                              </div>
                              <div className="mt-3 text-2xl font-black tracking-tight">{formatPrice(message.offer_amount)}</div>
                              <div className={`mt-2 text-sm ${isOwn ? 'text-white/72' : 'text-[#5B564C]'}`}>
                              ...
                              {canRespondToOffer ? (
                                <div className="mt-4 flex gap-2">
                                  <button
                                    ...
                                    className="inline-flex items-center gap-2 rounded-md bg-[#FFD600] px-4 py-2 text-xs font-black uppercase tracking-[0.16em] text-[#0D0D0D]"
                                  >
                                  ...
                                  <button
                                    ...
                                    className="inline-flex items-center gap-2 rounded-md border border-black/10 px-4 py-2 text-xs font-semibold text-[#0D0D0D]"
                                  >
                          ...
                          <div className={`mt-2 text-[11px] ${isOwn ? 'text-white/55' : 'text-[#8A8577]'}`}>{formatMarketplaceTime(message.created_at)}</div>
                        </div>
                      </div>
                    );
                  }) : (
                ...
                <div className="border-t border-black/8 bg-white p-4 sm:p-5">
                  <AnimatePresence>
                    {showOfferComposer ? (
                      <motion.div
                        ...
                        className="mb-3 overflow-hidden rounded-[24px] bg-[#FFFCF2] p-4"
                      >
                        <div className="text-xs font-semibold uppercase tracking-[0.18em] text-[#7C6C20]">Send an offer</div>
                        <div className="mt-3 flex gap-3">
                          <div className="relative flex-1">
                            <IndianRupee className="pointer-events-none absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-[#6B665B]" />
                            <input
                              value={offerAmount}
                              onChange={(event) => setOfferAmount(event.target.value.replace(/[^0-9]/g, ''))}
                              placeholder="Enter offer amount"
                              className="w-full rounded-md border border-black/10 bg-white px-10 py-3 text-sm outline-none"
                            />
                          </div>
                          <button
                            type="button"
                            ...
                            disabled={isSending}
                            className="rounded-md bg-[#FFD600] px-5 py-3 text-xs font-black uppercase tracking-[0.16em] text-[#0D0D0D] disabled:opacity-60"
                          >
                            Send offer
                          </button>
                        </div>
                  ...
                  <div className="flex flex-wrap gap-3">
                    <div className="flex min-w-[220px] flex-1 items-center rounded-md border border-black/10 bg-[#FFFCF5] px-4 py-3">
                      <input
                        value={messageDraft}
                    ...
                    <label className="inline-flex h-12 w-12 cursor-pointer items-center justify-center rounded-md border border-black/10 bg-white text-[#0D0D0D]">
                    ...
                    <button
                      ...
                      className={`rounded-md px-4 py-3 text-sm font-semibold ${showOfferComposer ? 'bg-[#0D0D0D] text-white' : 'border border-black/10 bg-white text-[#0D0D0D]'}`}
                    >
                      Offer
                    </button>
                    <button
                      ...
                      className="inline-flex items-center gap-2 rounded-md bg-[#FFD600] px-5 py-3 text-sm font-black uppercase tracking-[0.18em] text-[#0D0D0D] disabled:cursor-not-allowed disabled:opacity-60"
                    >
