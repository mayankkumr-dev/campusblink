...
export const AccountRestrictedPage: React.FC = () => {
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-[#FAFAF8] p-4">
      <div className="w-full max-w-xl rounded-lg border border-black/10 bg-white p-8 shadow-[0_24px_80px_rgba(15,23,42,0.08)]">
        <div className="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-md bg-[#FEE2E2] text-[#DC2626]">
          <AlertTriangle className="h-7 w-7" />
        </div>

        <h1 className="text-center font-syne text-3xl font-extrabold text-[#0D0D0D]">{heading}</h1>
        ...
        <div className="mt-6 rounded-lg border border-black/10 bg-[#FAFAF8] p-4 text-sm text-[#4A4A4A]">
        ...
        <div className="mt-6 flex flex-wrap items-center justify-center gap-3">
          <Link
            ...
            className="inline-flex items-center gap-2 rounded-md bg-[#0D0D0D] px-5 py-2.5 text-sm font-bold text-white transition-colors hover:bg-[#FFD600] hover:text-[#0D0D0D]"
          >
            <Mail className="h-4 w-4" /> Contact Admin
          </Link>
          <Link to="/" className="rounded-md border border-black/10 px-5 py-2.5 text-sm font-bold text-[#0D0D0D] hover:bg-[#FAFAF8]">
            Back to Home
          </Link>
        </div>
      </div>
