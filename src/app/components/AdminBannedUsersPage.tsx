    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="rounded-lg border border-black/[0.08] bg-white p-4">
        <div className="relative w-full md:w-96">
          ...
          <input value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} placeholder="Search banned users..." className="w-full rounded-lg border border-black/10 bg-[#F7F5F0] py-2 pl-9 pr-4 text-sm text-[#0D0D0D] outline-none focus:border-[#FFD600]" />
        </div>
      </div>

      {isLoading ? (
        <div className="flex justify-center py-16"><Loader2 className="h-7 w-7 animate-spin text-[#FFD600]" /></div>
      ) : users.length > 0 ? (
          ...
          <table className="w-full border-collapse text-left">
            <thead className="bg-[#F5F4F0] h-[40px] border-b border-[#E8E8E8]">
              <tr className="border-b border-black/[0.08] bg-[#F7F5F0] hover:bg-[#FAFAF8] transition-colors duration-150">
                ...
                <th className="p-4 text-xs font-bold uppercase tracking-wider text-[#6B6B6B] px-4 text-left font-sans font-semibold text-[12px] text-[#9B9B9B] uppercase tracking-[0.6px]">Banned At</th>
            ...
            <tbody className="divide-y divide-black/[0.06]">
              {users.map((user) => (
                <tr key={user.id} className="hover:bg-black/[0.03]">
                  <td className="p-4 text-sm text-[#0D0D0D]"><div className="font-bold">{user.name || 'Unnamed user'}</div><div className="text-xs text-[#6B6B6B]">{user.email}</div></td>
                  <td className="p-4 text-sm text-[#0D0D0D]">{user.ban_reason || 'No reason recorded'}</td>
                  <td className="p-4 text-xs text-[#6B6B6B]">{user.banned_at ? new Date(user.banned_at).toLocaleString() : 'Unknown'}</td>
        ...
        <div className="rounded-lg border border-dashed border-black/10 bg-white p-10 text-center text-sm text-[#6B6B6B]">No banned users found.</div>
      )}
    </div>
  );
};
