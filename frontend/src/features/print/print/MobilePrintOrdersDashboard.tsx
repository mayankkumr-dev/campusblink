...
export interface MobilePrintOrdersDashboardProps {
  shop: any;
  onOverride: (nextOverride: string | null) => Promise<void>;
  orders: any[];
  setOrders: React.Dispatch<React.SetStateAction<any[]>>;
  openPreview: (order: any) => void;
}

export const MobilePrintOrdersDashboard: React.FC<MobilePrintOrdersDashboardProps> = ({
  shop,
  onOverride,
  orders,
  setOrders,
  ...
  const [activeTab, setActiveTab] = useState<'new' | 'in_progress'>('new');
  ...
  const handleAccept = async (id: string) => {
    ...
    if (error) {
      ...
      setOrders((prev) =>
        prev.map((o) => (o.id === id ? { ...o, status: 'printing' } : o))
      );
    }
  };
  ...
  const handleMarkReady = async (id: string) => {
    ...
    if (error) {
      ...
      toast.success('Print job marked ready for pickup!');
      setOrders((prev) =>
  ...
  const handleDismissReady = async (id: string) => {
    ...
    if (error) {
      ...
      setOrders((prev) =>
        prev.map((o) => (o.id === id ? { ...o, status: 'collected' } : o))
      );
    }
  };
  ...
  return (
    <div className="flex flex-col min-h-full bg-[#FAFAFA] dark:bg-shop-bg-base text-gray-900 dark:text-shop-text-primary font-sans pb-6 select-none transition-colors">
      {/* ========================================================
          GLOBAL STICKY TOP HEADER & SHOP STATUS PANEL
      ======================================================== */}
      <header className="sticky top-0 z-40 bg-white/95 dark:bg-shop-bg-surface/95 backdrop-blur-md shadow-[0_2px_15px_rgba(0,0,0,0.04)] dark:shadow-none border-b border-gray-100 dark:border-shop-border-subtle transition-colors">
        <div className="px-4 pt-3.5 pb-3 flex items-center justify-between gap-3">
          <div className="min-w-0 flex-1">
            ...
            <p className="text-[11px] font-semibold text-gray-400 dark:text-shop-text-secondary truncate mt-1">
              {shop?.name || 'Campus Print Shop'}
            </p>
          </div>

          <div className="flex items-center gap-2.5 shrink-0">
            {/* Sleek Modern Status Pill & Toggle Switch */}
            <div
            ...
            >
              ...
              <span className="text-xs font-bold font-syne tracking-wide">
                {isOpen ? 'Accepting print jobs' : 'Closed'}
              </span>
              <div
          ...
          {tabs.map((tab) => {
            ...
            return (
              ...
              >
                ...
                {isActive && (
                  <motion.div
                    layoutId="printOrderTabUnderline"
                    className="absolute bottom-0 left-4 right-4 h-0.5 bg-gray-900 dark:bg-shop-accent rounded-full"
      ...
      <div className="px-4 pt-4">
        ...
        {activeTab === 'new' && (
          <div className="space-y-3.5">
            {newOrdersList.length === 0 ? (
              <div className="bg-white dark:bg-shop-bg-surface rounded-3xl p-8 text-center shadow-[0_4px_20px_rgba(0,0,0,0.03)] dark:shadow-none border border-gray-100 dark:border-shop-border-subtle my-6 transition-colors">
                ...
                <p className="text-xs text-gray-500 dark:text-shop-text-secondary font-medium mt-1 max-w-xs mx-auto">
                  All caught up! New document print requests will appear here automatically.
                </p>
              </div>
            ) : (
              newOrdersList.map((order) => {
                ...
                const isColor =
                  order.color_mode === 'color' || order.print_color === 'color';
                const copies = order.copies || order.quantity || 1;
                const orientation = order.orientation || 'Portrait';
                ...
                return (
                  <motion.div
                    ...
                    className="bg-white dark:bg-shop-bg-surface rounded-2xl p-4 shadow-[0_6px_24px_rgba(0,0,0,0.05)] dark:shadow-none border border-gray-100 dark:border-shop-border-subtle flex flex-col transition-colors"
                  >
                    ...
                    {/* Document Title */}
                    <div className="mt-3">
                      <p className="font-syne font-extrabold text-sm text-gray-900 dark:text-shop-text-primary truncate">
                        {order.file_name || 'Attached Document.pdf'}
                      </p>
                    </div>

                    {/* Minimalist Soft-Gray Tinted Chips Row for Document Specs */}
                    <div className="flex flex-wrap items-center gap-1.5 my-3">
                      <span className="px-2.5 py-1 rounded-lg bg-gray-50 dark:bg-shop-bg-surface-raised text-gray-700 dark:text-shop-text-primary text-[11px] font-bold font-syne">
                        {copies} {copies === 1 ? 'Copy' : 'Copies'}
                      </span>
                      <span
                        className={`px-2.5 py-1 rounded-lg text-[11px] font-bold font-syne ${
                          isColor
                            ? 'bg-purple-50 dark:bg-purple-950/40 text-purple-700 dark:text-purple-300'
                            : 'bg-gray-50 dark:bg-shop-bg-surface-raised text-gray-700 dark:text-shop-text-primary'
                        }`}
                      >
                        {isColor ? '🎨 Color Print' : '⚫ B&W Print'}
                      </span>
                      <span className="px-2.5 py-1 rounded-lg bg-gray-50 dark:bg-shop-bg-surface-raised text-gray-600 dark:text-shop-text-secondary text-[11px] font-semibold">
                        {orientation}
                      </span>
                      {order.pages && (
                        <span className="px-2.5 py-1 rounded-lg bg-gray-50 dark:bg-shop-bg-surface-raised text-gray-600 dark:text-shop-text-secondary text-[11px] font-semibold">
                    ...
                    {/* Download & Preview Links Cleanly Grouped Above Pricing */}
                    <div className="flex items-center justify-between pt-2 pb-3 border-t border-gray-100 dark:border-shop-border-subtle">
                      <div className="flex items-center gap-2">
                        <button
                          type="button"
                          onClick={() => openPreview(order)}
                          className="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 text-xs font-bold hover:bg-blue-100 transition-colors"
                        >
                        ...
                        {order.file_url && (
                          <a
                            ...
                            className="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-gray-100 dark:bg-shop-bg-surface-raised text-gray-700 dark:text-shop-text-secondary text-xs font-bold hover:bg-gray-200 transition-colors"
                          >
                      ...
                      <div className="text-right">
                        <span className="text-[10px] font-bold uppercase tracking-wider text-gray-400 dark:text-shop-text-secondary block">
                          Total
                        </span>
                        <span className="font-syne text-lg font-black text-gray-900 dark:text-shop-text-primary">
                          ₹{order.total}
                        </span>
                      </div>
                    </div>

                    {/* Actions: Premium Tactile Accept Primary & Sleek Ghost Reject */}
                    <div className="flex items-center justify-between gap-3 pt-1">
                      ...
                      <button
                        ...
                        className="flex-1 bg-blue-600 hover:bg-blue-700 active:scale-[0.97] text-white font-syne font-bold text-xs py-3 px-5 rounded-xl shadow-[0_4px_14px_rgba(37,99,235,0.25)] dark:shadow-none flex items-center justify-center gap-2 transition-all"
                      >
            ...
            {activeOrdersList.length === 0 ? (
              <div className="bg-white dark:bg-shop-bg-surface rounded-3xl p-8 text-center shadow-[0_4px_20px_rgba(0,0,0,0.03)] dark:shadow-none border border-gray-100 dark:border-shop-border-subtle my-6 transition-colors">
                <div className="w-14 h-14 rounded-2xl bg-emerald-50 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 flex items-center justify-center mx-auto mb-3">
                  <Printer className="w-6 h-6 stroke-[1.8]" />
                </div>
                <h3 className="font-syne text-base font-extrabold text-gray-900 dark:text-shop-text-primary">
                  No active print jobs
                </h3>
                <p className="text-xs text-gray-500 dark:text-shop-text-secondary font-medium mt-1 max-w-xs mx-auto">
                  Accepted print requests in progress or ready for pickup will show up here.
                </p>
              </div>
            ) : (
              activeOrdersList.map((order) => {
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between gap-3">
                      <div className="flex items-center gap-2.5 min-w-0">
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="text-[11px] font-medium text-gray-400 dark:text-shop-text-secondary mt-0.5 truncate">
                    ...
                    {/* Preview Button Row */}
                    <div className="flex items-center justify-between my-3 pt-2 border-t border-gray-100 dark:border-shop-border-subtle">
                      <span className="text-xs font-semibold text-gray-500 dark:text-shop-text-secondary">
                        {isReady ? '🎉 Printed & Ready' : '🖨️ Currently Printing'}
                      </span>
                      <button
                        type="button"
                        onClick={() => openPreview(order)}
                        className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg bg-gray-50 dark:bg-shop-bg-surface-raised text-gray-700 dark:text-shop-text-primary text-xs font-bold"
                      >
                        <Eye className="w-3.5 h-3.5" />
                    ...
                    {isReady ? (
                      /* Ready for Pickup Card: Prominent full-width Mark Collected button in light success-green tint */
                      <button
                        type="button"
