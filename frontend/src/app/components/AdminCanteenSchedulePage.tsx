...
export const AdminCanteenSchedulePage: React.FC = () => {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <button onClick={() => navigate('/admin/accounts?tab=canteens')} className="inline-flex items-center gap-2 rounded-lg border border-black/[0.08] bg-[var(--bg)] px-4 py-2 text-sm font-bold text-[var(--text-primary)] hover:bg-[var(--bg-tertiary)]">
        <ArrowLeft className="h-4 w-4" /> Back to canteens
      </button>

      <div className="rounded-lg border border-black/[0.08] bg-[var(--bg)] p-6">
        <h1 className="font-syne text-2xl font-bold text-[var(--text-primary)]">{shop?.name || 'Canteen'} schedule</h1>
