            ...
            <div className="min-w-0 flex-1">
              ...
              <p
                className={`text-[11px] flex items-center gap-1.5 mt-1 font-semibold ${
                  shop?.is_open_now ? 'text-emerald-600' : 'text-rose-600'
                }`}
              >
                <span
                  className={`w-2 h-2 rounded-full ${
                    shop?.is_open_now ? 'bg-emerald-500 animate-pulse' : 'bg-rose-500'
                  }`}
                />
                        ...
                        <tbody className="divide-y divide-slate-100">
                          {historyOrders.map((order, i) => (
                            ...
                            >
                              ...
                              <td className="whitespace-nowrap px-6 py-4 text-right font-syne text-sm font-extrabold text-slate-900">
                                {order.status === 'completed' || order.status === 'picked_up' ? (
                                  `₹${order.total}`
                                ) : (
