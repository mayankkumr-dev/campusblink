  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
        <div className="rounded-lg border border-black/[0.08] bg-[var(--bg)] p-4"><div className="text-sm text-[var(--text-secondary)]">Total Orders Today</div><div className="font-syne text-2xl font-bold text-[var(--text-primary)]">{stats.totalOrders}</div></div>
        <div className="rounded-lg border border-black/[0.08] bg-[var(--bg)] p-4"><div className="text-sm text-[var(--text-secondary)]">Revenue Today</div><div className="font-syne text-2xl font-bold text-[#16A34A]">₹{stats.totalRevenue.toLocaleString()}</div></div>
        <div className="rounded-lg border border-black/[0.08] bg-[var(--bg)] p-4"><div className="text-sm text-[var(--text-secondary)]">Cancelled</div><div className="font-syne text-2xl font-bold text-[var(--text-primary)]">{stats.cancelled} <span className="text-sm text-[var(--text-secondary)]">({stats.cancellationRate}%)</span></div></div>
        <div className="rounded-lg border border-black/[0.08] bg-[var(--bg)] p-4"><div className="text-sm text-[var(--text-secondary)]">Avg Completion</div><div className="font-syne text-2xl font-bold text-[var(--yellow)]">{stats.avgCompletionMins}m</div></div>
      </div>
        ...
        <table className="w-full border-collapse text-left">
          <thead className="bg-[var(--bg-secondary)] h-[40px] border-b border-[var(--border)]">
            <tr className="border-b border-black/[0.08] bg-[var(--bg-tertiary)] hover:bg-[var(--bg-primary)] transition-colors duration-150">
              ...
              <th className="p-4 text-xs uppercase tracking-wider text-[var(--text-secondary)] px-4 text-left font-sans font-semibold text-[12px] text-[var(--text-muted)] uppercase tracking-[0.6px]">Canteen</th>
              <th className="p-4 text-xs uppercase tracking-wider text-[var(--text-secondary)] px-4 text-left font-sans font-semibold text-[12px] text-[var(--text-muted)] uppercase tracking-[0.6px]">Total</th>
          ...
          <tbody className="divide-y divide-black/[0.06]">
            {filteredOrders.map((order) => (
              <tr key={order.id} className="hover:bg-black/[0.03]">
                ...
                <td className="p-4 text-sm font-bold text-[var(--text-primary)]">₹{Number(order.total_amount || 0).toLocaleString()}</td>
                <td className="p-4">
                  <span className="rounded bg-[var(--bg-tertiary)] px-2 py-1 text-[10px] font-bold uppercase tracking-wider text-[var(--text-primary)]">{prettyStatus(order.status)}</span>
                </td>
            ...
            {filteredOrders.length === 0 && (
              <tr>
                <td colSpan={6} className="p-10 text-center text-sm text-[var(--text-secondary)]">No canteen orders found for current filters.</td>
              </tr>
            )}
          </tbody>
        </table>
