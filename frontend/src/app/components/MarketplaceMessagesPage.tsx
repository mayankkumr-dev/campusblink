...
export function MarketplaceMessagesPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50/50 px-4 py-6 sm:px-6 lg:px-8 font-sans">
      <div className="mx-auto max-w-[1400px] h-[calc(100vh-3rem)] flex flex-col gap-6 animate-in fade-in duration-500">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 shrink-0">
          <div>
            <h1 className="text-2xl md:text-3xl font-extrabold text-slate-900 tracking-tight font-syne">Messages</h1>
            <p className="text-sm text-slate-500 mt-1 font-medium">Manage your marketplace conversations and offers.</p>
          </div>
        </div>

        {/* Main Content Container */}
        <div className="flex-1 overflow-hidden bg-white rounded-[2rem] border border-slate-200/60 shadow-[0_8px_30px_rgb(0,0,0,0.04)] flex flex-col lg:flex-row">
          
          {/* Sidebar: Conversation List */}
          <div className="w-full lg:w-[380px] flex flex-col border-b lg:border-b-0 lg:border-r border-slate-100 shrink-0 bg-white/50">
            ...
            <div className="p-5 border-b border-slate-100 space-y-4 shrink-0 bg-white">
              ...
              <div className="flex p-1 bg-slate-100/70 rounded-2xl">
                <button 
                  ...
                  className={`flex-1 flex items-center justify-center gap-2 py-2.5 text-sm font-bold rounded-xl transition-all ${activeTab === 'inbox' ? 'bg-white text-slate-900 shadow-xs' : 'text-slate-500 hover:text-slate-700'}`}
                >
                ...
                <button 
                  ...
                  className={`flex-1 flex items-center justify-center gap-2 py-2.5 text-sm font-bold rounded-xl transition-all ${activeTab === 'requests' ? 'bg-white text-slate-900 shadow-xs' : 'text-slate-500 hover:text-slate-700'}`}
                >
              ...
              <div className="relative">
                ...
                <input 
                  ...
                  className="w-full bg-slate-50 border border-slate-200/60 rounded-xl py-2.5 pl-10 pr-4 text-sm font-medium outline-none focus:bg-white focus:border-amber-400 focus:ring-4 focus:ring-amber-500/10 transition-all text-slate-900 placeholder:text-slate-400"
                />
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto hide-scrollbar p-3 space-y-1">
              {isLoading ? (
                  ...
                  {new Array(5).fill(null).map((_, index) => (
                    <div key={index} className="rounded-2xl p-4 animate-pulse flex gap-4">
                      ...
                      <div className="flex-1 space-y-2 py-1">
                        ...
                        <div className="h-3 w-4/5 rounded-md bg-slate-100/60" />
                      </div>
                    </div>
                  ))}
                </div>
                  ...
                  return (
                    ...
                    >
                      <div className="relative shrink-0">
                        ...
                        {unreadCount > 0 && (
                          <div className="absolute -top-1 -right-1 min-w-[20px] h-5 rounded-full bg-rose-500 border-2 border-white flex items-center justify-center text-[10px] font-bold text-white px-1">
                            {unreadCount}
                          </div>
                        )}
                      </div>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between gap-2 mb-0.5">
                          <div className="truncate text-sm font-bold text-slate-900">{getProfileName(otherProfile)}</div>
                          <div className={`text-[10px] font-semibold whitespace-nowrap ${isActive ? 'text-amber-700' : 'text-slate-400'}`}>
                            {formatMarketplaceTime(conversation.last_message_at)}
                          </div>
                        </div>
                        <div className="truncate text-[11px] font-semibold text-slate-500 mb-1">
                ...
                <div className="flex flex-col items-center justify-center h-full p-6 text-center opacity-70">
                  <MessageCircle className="w-10 h-10 text-slate-300 mb-3 stroke-[1.5]" />
          ...
          <div className="flex-1 flex flex-col min-w-0 bg-slate-50/30">
            {activeConversation ? (
              <>
                {/* Chat Header */}
                <div className="h-[76px] flex items-center justify-between gap-4 px-6 border-b border-slate-100 bg-white shrink-0">
                  <div className="flex items-center gap-4">
                    {/* Mobile back button placeholder (can be implemented later) */}
                    <div className="w-10 h-10 rounded-full border border-slate-200 bg-slate-50 overflow-hidden shrink-0 hidden sm:block">
                    ...
                    <div>
                      <h2 className="text-base font-bold text-slate-900 tracking-tight">{getProfileName(peerProfile)}</h2>
                  ...
                  <div className="flex items-center gap-3">
                    <Link 
                      ...
                      className="hidden sm:flex items-center gap-2 px-4 py-2 rounded-xl border border-slate-200 bg-white text-xs font-bold text-slate-700 hover:bg-slate-50 transition-all shadow-xs"
                    >
                      <FileText className="w-3.5 h-3.5 text-slate-400" />
                      View Listing
                    </Link>
                    <button className="w-9 h-9 flex items-center justify-center rounded-full hover:bg-slate-100 text-slate-400 transition-colors">
                ...
                <div className="px-6 py-3 border-b border-slate-100 bg-white/60 backdrop-blur-sm shrink-0 flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl bg-slate-100 overflow-hidden border border-slate-200/60 shrink-0">
                  ...
                  <div className="flex-1 min-w-0 flex items-center justify-between">
                    <div className="truncate text-sm font-bold text-slate-800">{activeConversation.listing?.title}</div>
                    <div className="text-sm font-extrabold text-slate-900 bg-slate-100 px-2 py-1 rounded-md">{formatPrice(activeConversation.listing?.price)}</div>
                  </div>
                </div>
                ...
                <div className="flex-1 overflow-y-auto px-4 py-6 sm:px-6 space-y-6">
                  {isLoadingMessages ? (
                      ...
                      return (
                        <div key={message.id} className={`flex ${isOwn ? 'justify-end' : 'justify-start'}`}>
                          <div className="flex flex-col gap-1 max-w-[80%] sm:max-w-[70%]">
                            ...
                            >
                              ...
                              {message.message_type === 'offer' ? (
                                <div className="min-w-[200px]">
                                  <div className={`inline-flex items-center gap-1.5 rounded-lg px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider ${isOwn ? 'bg-white/20 text-white' : 'bg-amber-100/50 text-amber-700'}`}>
                                    <IndianRupee className="h-3 w-3" />
                                    Offer Received
                                  </div>
                                  <div className="mt-2 text-2xl font-black tracking-tight">{formatPrice(message.offer_amount)}</div>
                                  <div className={`mt-1 text-xs font-medium ${isOwn ? 'text-white/70' : 'text-slate-500'}`}>
                                  ...
                                  {canRespondToOffer ? (
                                    <div className="mt-4 flex flex-col gap-2">
                                      <button
                                        ...
                                        className="w-full flex justify-center items-center gap-2 rounded-xl bg-emerald-500 hover:bg-emerald-600 transition-colors px-4 py-2.5 text-xs font-bold text-white shadow-xs"
                                      >
                                      ...
                                      <button
                                        ...
                                        className="w-full flex justify-center items-center gap-2 rounded-xl bg-white text-slate-700 hover:bg-slate-50 transition-colors px-4 py-2.5 text-xs font-bold shadow-xs border border-slate-200"
                                      >
                            ...
                            <div className={`text-[10px] font-medium text-slate-400 ${isOwn ? 'text-right pr-1' : 'text-left pl-1'}`}>
                    ...
                    <div className="h-full flex flex-col items-center justify-center p-8 text-center space-y-4">
                       ...
                       <div>
                         <h3 className="text-base font-bold text-slate-900">Start the conversation</h3>
                         <p className="text-sm text-slate-500 mt-1 max-w-sm">Ask about the condition, pickup point, or send a quick offer to get started.</p>
                       </div>
                    </div>
                  )}
                </div>
                ...
                <div className="p-4 bg-white border-t border-slate-100 shrink-0">
                        ...
                        <div className="p-4 bg-amber-50/50 border border-amber-200/50 rounded-2xl flex flex-col sm:flex-row gap-3">
                           <div className="flex-1 relative">
                             ...
                             <input
                               ...
                               className="w-full bg-white border border-slate-200/60 rounded-xl py-2.5 pl-9 pr-4 text-sm font-bold text-slate-900 outline-none focus:border-amber-400 focus:ring-4 focus:ring-amber-500/10 transition-all"
                             />
                           </div>
                           <button
                             type="button"
                  ...
                  <div className="flex items-center gap-2">
                    <label className="shrink-0 w-11 h-11 flex items-center justify-center rounded-xl bg-slate-50 hover:bg-slate-100 text-slate-400 transition-colors cursor-pointer border border-slate-200/60">
                      <ImageIcon className="w-5 h-5" />
                      <input type="file" accept="image/*" className="hidden" onChange={handleUploadImage} />
                    </label>
                    <div className="flex-1 flex items-center rounded-2xl border border-slate-200 bg-slate-50 px-4 focus-within:bg-white focus-within:border-slate-300 focus-within:ring-4 focus-within:ring-slate-100 transition-all h-12">
                      <input
                        value={messageDraft}
                    ...
                    <button
                      ...
                      className={`shrink-0 px-4 h-12 rounded-xl text-sm font-bold transition-all shadow-xs flex items-center gap-2 ${showOfferComposer ? 'bg-slate-900 text-white' : 'bg-white border border-slate-200 text-slate-700 hover:bg-slate-50'}`}
                    >
              ...
              <div className="h-full flex flex-col items-center justify-center p-8 bg-slate-50/50">
                 ...
                 <h2 className="text-xl font-extrabold text-slate-900 mb-2">No conversation selected</h2>
