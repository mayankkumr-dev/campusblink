              ...
              <tr key={shop.id} className="hover:bg-black/[0.03] transition-colors group">
                <td className="p-4 min-w-[200px]">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-lg bg-slate-900 flex items-center justify-center font-syne font-bold text-lg text-amber-500 border border-black/[0.08] shrink-0 shadow-inner overflow-hidden">
                      {shop.logo_url ? (
                    ...
                    <div>
                      <div className="font-sans font-bold text-sm text-slate-900 mb-0.5">{shop.name}</div>
                      <div className="font-sans text-xs text-slate-500">{shop.college}</div>
                    </div>
                  </div>
                </td>
                <td className="p-4">
                   <div className="font-sans text-sm text-slate-900 font-medium mb-0.5">{shop.owner?.name || 'Unknown'}</div>
                   <div className="font-sans text-xs text-slate-500">{shop.owner?.email || 'No email associated'}</div>
                </td>
                <td className="p-4">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="bg-white border border-black/10 rounded-lg w-full max-w-2xl max-h-[90vh] overflow-y-auto relative shadow-md animate-in zoom-in-95 duration-200 hide-scrollbar">
            ...
            <div className="sticky top-0 bg-white/90  border-b border-black/[0.08] p-6 flex items-center justify-between z-10">
              <h2 className="font-syne font-bold text-xl text-slate-900">{isEditMode ? 'Edit Print Shop' : 'Register Print Shop'}</h2>
            ...
            <form onSubmit={handleCreateShop} className="p-6 space-y-6">
              <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
                <div className="md:col-span-2">
                  <label className="mb-2 block text-sm font-bold text-slate-900">Shop Name</label>
                  <input
                    value={form.name}
                ...
                <div>
                  <label className="mb-2 block text-sm font-bold text-slate-900">B/W Price Per Page</label>
                  <input
                    type="number"
                    min="0"
                    step="1"
                ...
                <div>
                  <label className="mb-2 block text-sm font-bold text-slate-900">Color Price Per Page</label>
                  <input
                    type="number"
                    min="0"
                    step="1"
                ...
                <div>
                  <label className="mb-2 block text-sm font-bold text-slate-900">Binding Charge</label>
                  <input
                    type="number"
                    min="0"
                    step="1"
