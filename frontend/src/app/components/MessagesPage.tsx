...
const CtxMenuPopover: React.FC<{
 ...
 const btnClass = 'w-full flex items-center gap-3 px-4 py-3 text-sm font-medium text-gray-700 hover:bg-gray-50 :bg-gray-800 transition-colors text-left';
 return (
 <div className="fixed inset-0 z-50" onClick={onClose}>
 <div
 className="absolute bg-white rounded-2xl shadow-[0_8px_40px_rgba(0,0,0,0.14)] border border-gray-100 py-1.5 w-[188px] overflow-hidden"
 ...
 <button className={`${btnClass} text-red-500 hover:bg-red-50 :bg-red-950`} onClick={() => { onDelete(); onClose(); }}>
...
const BottomSheet: React.FC<{
 ...
 const btnClass = 'w-full flex items-center gap-4 px-5 py-4 rounded-2xl text-sm font-semibold text-gray-800 hover:bg-gray-50 :bg-gray-800 active:bg-gray-100 transition-colors text-left';
 return (
 <div
 ...
 >
 <div
 className="w-full bg-white rounded-t-3xl shadow-[0_-8px_40px_rgba(0,0,0,0.12)] overflow-hidden"
...
const DeleteModal: React.FC<{
 ...
 >
 <div
 className="bg-white rounded-3xl shadow-[0_20px_60px_rgba(0,0,0,0.2)] border border-gray-100 w-full max-w-[320px] p-6"
 onClick={e => e.stopPropagation()}
 >
 <div className="w-12 h-12 bg-red-50 rounded-2xl flex items-center justify-center mb-4">
 <Trash2 className="h-5 w-5 text-red-500" />
 </div>
 <h3 className="font-syne text-base font-extrabold text-gray-900 mb-1">
 ...
 <button
 className="w-full py-3 text-sm font-bold text-gray-700 bg-gray-50 hover:bg-gray-100 :bg-gray-700 rounded-2xl border border-gray-200 transition-colors"
 onClick={() => onDelete('me')}
 >
 Delete for me
 </button>
 {modal.canDeleteForEveryone && (
 <button
 className="w-full py-3 text-sm font-bold text-red-600 bg-red-50 hover:bg-red-100 :bg-red-900 rounded-2xl border border-red-100 transition-colors"
 ...
 <button
 className="w-full py-2.5 text-sm font-semibold text-gray-400 hover:text-gray-600 :text-gray-300 transition-colors"
 onClick={onClose}
 >
 Cancel
 </button>
...
export const MessagesPage: React.FC = () => {
 ...
 const loadConversations = useCallback(async () => {
   if (profile?.id) {
     ...
     return { active: state.activeConversations, requests: state.requestConversations };
   }
 ...
 // ── Supabase Realtime via Zustand ─────────────────────────────────────────
 useEffect(() => {
   if (!profile?.id) return;
   const cleanup = setupRealtimeSubscription(profile.id);
   return () => cleanup();
 ...
 useEffect(() => {
   if (activeChatId) {
     ...
     fetchMessagesForChat(activeChatId).then(() => {
       setTimeout(() => messagesEndRef.current?.scrollIntoView({ behavior: 'auto' }), 100);
     });
   }
 ...
 if (newConv) { setActiveChatId(newConv._id); addMessage(newConv._id, data.message); setSearchParams({ chat: newConv._id }, { replace: true }); }
 setTimeout(() => messagesEndRef.current?.scrollIntoView({ behavior: 'smooth' }), 100);
 ...
 if (activeChatId) updateMessage(activeChatId, editingId, { text: updated.text, editedAt: updated.editedAt });
 setEditingId(null);
 ...
 if (activeChatId) removeMessages(activeChatId, ids, deleteFor === 'everyone' ? 'everyone' : profile!.id);
 setDeleteModal(null);
 exitSelectionMode();
 ...
 // ─────────────────────────────────────────────────────────────────────────
 return (
 <div className="flex h-[calc(100vh-64px)] md:h-[100dvh] w-full overflow-hidden bg-gray-50 text-gray-900 font-sans">
 ...
 {/* ── LEFT PANE ───────────────────────────────────────────────────── */}
 <div className={`flex flex-col w-full md:w-[380px] lg:w-[420px] flex-shrink-0 border-r border-gray-200 bg-white ${rightPaneOpen ? 'hidden md:flex' : 'flex'}`}>
 <div className="px-6 pt-6 pb-4 bg-gray-50/80 backdrop-blur-md sticky top-0 z-10 border-b border-gray-200">
 ...
 className="w-full bg-white hover:bg-white focus:bg-gray-50 text-gray-900 text-sm rounded-2xl pl-10 pr-4 py-2.5 outline-none focus:ring-4 focus:ring-accent-blue-soft border border-transparent transition-all"
 />
 </div>
 </div>

 <div className="flex items-center px-6 py-2 gap-6 border-b border-gray-200 bg-gray-50">
 ...
 className={`pb-3 text-sm font-bold transition-all relative flex items-center gap-1.5 capitalize ${activeTab === tab ? 'text-gray-900' : 'text-gray-500 hover:text-gray-900'}`}
 >
 {tab}
 ...
 {newChatDetails && (
 <div className="w-full flex items-center gap-4 p-3 rounded-2xl bg-white shadow-sm border border-accent-blue/30 mb-2">
 <img src={newChatDetails.avatar} alt={newChatDetails.name} className="h-12 w-12 rounded-full object-cover border border-gray-200" />
 <div className="flex-1 min-w-0">
 <h3 className="text-sm font-bold text-gray-900 font-syne truncate">{newChatDetails.name}</h3>
 <p className="text-xs text-accent-blue font-semibold">New conversation</p>
 </div>
 </div>
 )}
 {filteredConvs.length === 0 && !newChatDetails ? (
 <div className="text-center mt-10 text-gray-500 text-sm font-medium">No conversations yet.</div>
 ...
 className={`w-full flex items-center gap-4 p-3 rounded-2xl transition-all text-left mb-1 ${activeChatId === details.id ? 'bg-white shadow-sm border border-gray-200' : 'hover:bg-white border border-transparent'}`}
 >
 <img src={details.avatar} alt={details.name} className="h-12 w-12 rounded-full object-cover border border-gray-200 shadow-sm flex-shrink-0" />
 <div className="flex-1 min-w-0">
 <div className="flex items-center justify-between mb-0.5">
 <h3 className="text-sm truncate pr-2 font-syne font-bold text-gray-900/90">{details.name}</h3>
 <span className="text-[11px] font-semibold flex-shrink-0 text-gray-500/70">{details.timestamp}</span>
 </div>
 {isSenderWaiting ? (
 ...
 <p className="text-xs truncate font-medium text-gray-500">{details.lastMessage || 'No messages yet'}</p>
 )}
 </div>
 </button>
 );
 ...
 {/* ── RIGHT PANE ──────────────────────────────────────────────────── */}
 <div className={`flex-1 flex flex-col bg-gray-50 relative ${!rightPaneOpen ? 'hidden md:flex' : 'flex'}`}>
 ...
 <h2 className="font-syne text-2xl font-extrabold text-gray-900 mb-2">Campus Messages</h2>
 <p className="text-gray-500 text-sm font-medium max-w-sm text-center">Select a conversation or check your message requests.</p>
 </div>
 )}
 ...
 <>
 <div className="h-[72px] flex items-center justify-between px-6 border-b border-gray-200 bg-gray-50/90 backdrop-blur-md sticky top-0 z-10 flex-shrink-0">
 <div className="flex items-center gap-4">
 <button onClick={() => { setNewChatUserId(null); setNewChatProfile(null); }} className="md:hidden h-9 w-9 flex items-center justify-center rounded-full bg-white text-gray-500 hover:bg-white transition-colors">
 <ArrowLeft className="h-5 w-5" />
 </button>
 <img src={newChatDetails.avatar} alt={newChatDetails.name} className="h-10 w-10 rounded-full object-cover border border-gray-200" />
 <div>
 <h2 className="font-syne text-base font-bold text-gray-900 leading-tight">{newChatDetails.name}</h2>
 {newChatDetails.subtitle && <p className="text-xs text-gray-500">{newChatDetails.subtitle}</p>}
 </div>
 </div>
 </div>
 ...
 <h3 className="font-syne font-extrabold text-gray-900 text-lg mb-1">{newChatDetails.name}</h3>
 <p className="text-sm text-gray-500 text-center max-w-xs">Send a message to start the conversation!</p>
 </div>
 <div className="p-4 bg-gray-50 border-t border-gray-200 flex-shrink-0">
 <div className="flex items-end gap-2 max-w-4xl mx-auto">
 <div className="flex-1 bg-white border border-gray-200 rounded-3xl flex items-center shadow-sm focus-within:ring-4 focus-within:ring-accent-blue-soft focus-within:border-accent-blue transition-all px-2 overflow-hidden">
 <button className="h-9 w-9 flex-shrink-0 flex items-center justify-center text-gray-500"><Smile className="h-5 w-5" /></button>
 <textarea ref={textareaRef} autoFocus placeholder={`Message ${newChatDetails.name}...`}
 className="flex-1 bg-transparent border-none focus:ring-0 resize-none text-sm font-medium text-gray-900 py-3.5 px-2 outline-none max-h-[120px] scrollbar-hide min-h-[48px]"
 rows={1} value={inputText} onChange={e => setInputText(e.target.value)} onKeyDown={handleKeyDown}
 />
 </div>
 <button onClick={handleSend} disabled={!inputText.trim()}
 className={`h-11 w-11 flex-shrink-0 flex items-center justify-center rounded-full transition-all ${inputText.trim() ? 'bg-blue-600 text-white shadow-[0_4px_14px_rgba(37,99,235,0.3)] hover:bg-blue-700' : 'bg-white text-gray-500'}`}>
 ...
 <>
 {/* Chat Header */}
 <div className={`h-[72px] flex items-center justify-between px-6 border-b border-gray-200 backdrop-blur-md sticky top-0 z-10 flex-shrink-0 transition-colors ${isSelectionMode ? 'bg-blue-50 ' : 'bg-gray-50/90'}`}>
 {isSelectionMode ? (
 <>
 <div className="flex items-center gap-4">
 <button onClick={exitSelectionMode} className="h-9 w-9 flex items-center justify-center rounded-full bg-white text-gray-500 hover:bg-white transition-colors">
 <X className="h-5 w-5" />
 </button>
 <span className="font-syne font-bold text-gray-900 text-base">{selectedIds.size} selected</span>
 </div>
 <button
 onClick={() => selectedIds.size > 0 && openDeleteModal(Array.from(selectedIds))}
 disabled={selectedIds.size === 0}
 className={`h-10 w-10 flex items-center justify-center rounded-full transition-all ${selectedIds.size > 0 ? 'bg-red-50 text-red-500 hover:bg-red-100 :bg-red-900' : 'text-gray-500/40'}`}
 >
 ...
 <>
 <div className="flex items-center gap-4">
 <button onClick={() => { setActiveChatId(null); setSearchParams({}, { replace: true }); }} className="md:hidden h-9 w-9 flex items-center justify-center rounded-full bg-white text-gray-500 hover:bg-white transition-colors">
 <ArrowLeft className="h-5 w-5" />
 </button>
 <img src={activeChatDetails.avatar} alt={activeChatDetails.name} className="h-10 w-10 rounded-full object-cover border border-gray-200" />
 <h2 className="font-syne text-base font-bold text-gray-900 leading-tight">{activeChatDetails.name}</h2>
 </div>
 <button className="h-10 w-10 flex items-center justify-center rounded-full text-gray-500 hover:text-gray-900 hover:bg-white transition-colors">
 ...
 {/* Messages Body */}
 <div className="flex-1 overflow-y-auto p-6 bg-white/30 flex flex-col gap-3"
 onClick={() => { if (contextMenu) setContextMenu(null); }}
 >
 {visibleMessages.length === 0 ? (
 <div className="text-center mt-10 text-gray-500 text-sm font-medium">Say hello! 👋</div>
 ...
 {/* Selection checkbox */}
 {isSelectionMode && (
 <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 transition-all ${isSelected ? 'bg-blue-600 border-blue-600 shadow-[0_0_0_3px_rgba(37,99,235,0.15)]' : 'border-gray-300 bg-white '}`}>
 ...
 {isEditing ? (
 /* Inline Edit */
 <div className="bg-white border-2 border-blue-400 rounded-2xl px-3 py-2 shadow-md w-72">
 <textarea
 ...
 className="w-full text-sm resize-none outline-none border-none bg-transparent text-gray-900 leading-relaxed" rows={2}
 />
 ...
 : 'bg-white border border-gray-200 text-gray-900 rounded-[22px] rounded-bl-sm'
 }`}>
 {msg.text}
 </div>
 {msg.editedAt && (
 <span className="text-[10px] text-gray-500/50 mt-0.5 px-1">Edited</span>
 )}
 </>
 )}
 ...
 <span className="text-[10px] font-semibold text-gray-500/60">
 ...
 <button
 className={`absolute top-1.5 opacity-0 group-hover:opacity-100 transition-opacity z-10 h-7 w-7 rounded-full bg-white shadow-md border border-gray-100 flex items-center justify-center ${isMe ? '-left-9' : '-right-9'}`}
 ...
 {iAmTheSenderWaiting ? (
 <div className="p-4 bg-gray-50 border-t border-gray-200 flex-shrink-0">
 <div className="max-w-4xl mx-auto flex items-center justify-between gap-3 bg-white border border-gray-200 rounded-2xl px-5 py-3.5">
 <div className="flex items-center gap-3">
 <div className="w-2 h-2 rounded-full bg-amber-400 animate-pulse flex-shrink-0" />
 <p className="text-sm font-medium text-gray-500">
 Waiting for <span className="font-bold text-gray-900">{activeChatDetails.name}</span> to accept your request...
 </p>
 </div>
 ...
 className="flex-1 bg-red-50 hover:bg-red-100 text-red-600 font-bold py-3 rounded-2xl text-sm transition-colors border border-red-100 ">
 Delete Request
 </button>
 ...
 <div className="p-4 bg-gray-50 border-t border-gray-200 flex-shrink-0">
 <div className="flex items-end gap-2 max-w-4xl mx-auto">
 <button className="h-11 w-11 flex-shrink-0 flex items-center justify-center rounded-full text-gray-500 hover:text-accent-blue hover:bg-accent-blue-soft transition-colors">
 <Paperclip className="h-5 w-5" />
 </button>
 <div className="flex-1 bg-white border border-gray-200 rounded-3xl flex items-center shadow-sm focus-within:ring-4 focus-within:ring-accent-blue-soft focus-within:border-accent-blue transition-all px-2 overflow-hidden">
 <button className="h-9 w-9 flex-shrink-0 flex items-center justify-center text-gray-500 hover:text-gray-900 transition-colors">
