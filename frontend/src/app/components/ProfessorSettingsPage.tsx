...
import { LogOut, Shield, KeyRound, MessageSquare, Megaphone, ChevronDown, ChevronRight, Sun, Moon, CheckCircle2, Eye, EyeOff, Check, ArrowRight, ShieldCheck, MessageSquareHeart, Monitor, Calendar, Upload, FileText, Loader2, RefreshCw, Plus, Trash2, Save, Edit3 } from 'lucide-react';
...
export const ProfessorSettingsPage: React.FC = () => {
  ...
  useEffect(() => {
    async function loadSchedule() {
      try {
        ...
        const res = await getProfessorSchedule();
  ...
  const handleSaveManualSchedule = async () => {
    ...
    const res = await saveProfessorSchedule(manualSlots);
  ...
  const handleScheduleUpload = async (file: File) => {
    ...
    const res = await uploadProfessorScheduleFile(file);
    if (res.error) {
